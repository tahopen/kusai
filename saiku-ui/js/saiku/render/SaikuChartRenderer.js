var KusaiChartRenderer = function (data, options) {
	...
	if (!options && !options.hasOwnProperty("htmlObject")) {
		throw "You need to supply a html object in the options for the KusaiChartRenderer!";
	}
	this.el = $(options.htmlObject);
	this.id = _.uniqueId("chart_");
	$(this.el).html(
		'<div class="canvas_wrapper" style="display:none;"><div id="canvas_' +
			this.id +
			'"></div></div>'
	);
	...
	if (options.zoom) {
		var self = this;
		var btns =
			"<span style='float:left;' class='zoombuttons'><a href='#' class='button rerender i18n' title='Re-render chart'></a><a href='#' class='button zoomout i18n' style='display:none;' title='Zoom back out'></a></span>";
...
KusaiChartRenderer.prototype.adjust = function () {
	var self = this;
...
KusaiChartRenderer.prototype.zoomin = function () {
	$(this.el).find(".canvas_wrapper").hide();
	var chart = this.chart.root;
	var data = chart.data;
	data.datums(null, { selected: false }).each(function (datum) {
...
KusaiChartRenderer.prototype.zoomout = function () {
	var chart = this.chart.root;
	var data = chart.data;
...
KusaiChartRenderer.prototype.render = function () {
	_.delay(this.render_chart_element, 0, this);
};

KusaiChartRenderer.prototype.switch_chart = function (key, override) {
	if (
...
KusaiChartRenderer.prototype.sunburst = function (o) {
	this.type = "sunburst";
	...
	partition.node
		.add(pv.Wedge)
		.fillStyle(
			pv.colors(options.colors).by(function (d) {
				return d.parentNode && d.parentNode.nodeName;
			})
		)
		.visible(function (d) {
...
KusaiChartRenderer.prototype.cccOptionsDefault = {
	Base: {
		animate: false,
		selectable: true,
		valuesVisible: false,
...
KusaiChartRenderer.prototype.getQuickOptions = function (baseOptions) {
...
KusaiChartRenderer.prototype.define_chart = function (displayOptions) {
	if (!this.hasProcessed) {
		this.process_data_tree({ data: this.rawdata }, true, true);
	}

	var self = this;
	var workspaceResults = this.adjustSizeTo
		? $(this.adjustSizeTo)
		: $(this.el);
	var isSmall =
	...
	var zoomDefinition = {
		...
		userSelectionAction: function (selectingDatums) {
			...
			if (data.datums().count() > 1500) {
				...
				data.datums(null, { visible: true }).count() ==
				data.datums().count()
			) {
				...
				var newSelection = [];
				_.each(
					_.difference(visibleOnes, selectingDatums),
					function (datum) {
						datum.setVisible(false);
					}
				);
				_.each(
					_.intersection(visibleOnes, selectingDatums),
					function (datum) {
	...
	if (runtimeChartDefinition.type === "TimeWheel") {
		...
		if (this.data.metadata.length > 2) {
			runtimeChartDefinition.readers =
				this.data.metadata[0].colName +
				", " +
		...
		this.chart = new pvc[runtimeChartDefinition.type](
			runtimeChartDefinition
		);
		this.chart.setData(this.data, {
			crosstabMode: true,
...
KusaiChartRenderer.prototype.render_chart_element = function (context) {
...
KusaiChartRenderer.prototype.process_data_tree = function (
	args,
	flat,
	setdata
) {
	...
	if (cellset && cellset.length > 0) {
		...
		for (
			...
			for (
				var field = 0, fieldLen = cellset[row].length;
				field < fieldLen;
				field++
			) {
				if (!hasStart) {
				...
				if (cellset[row][field].type == "COLUMN_HEADER") {
					...
					while (lowest_col_header <= row) {
						if (
							cellset[lowest_col_header][field].value !== "null"
						) {
							colheader.push(
								cellset[lowest_col_header][field].value
							);
						}
						lowest_col_header++;
					}
		...
		for (row = data_start, rowLen = cellset.length; row < rowLen; row++) {
			if (cellset[row][0].value !== "") {
				...
				for (labelCol = 0; labelCol <= lowest_level; labelCol++) {
					if (
						cellset[row] &&
						cellset[row][labelCol].value === "null"
					) {
						currentDataPos = data;
						var prevLabel = 0;
						for (
							...
							currentDataPos =
								currentDataPos[rowlabels[prevLabel]];
						}
				...
				for (
					var col = lowest_level + 1, colLen = cellset[row].length;
					col < colLen;
					col++
				) {
					...
					if (raw && raw !== "null") {
						...
						value = parseFloat(
							cell.value.replace(/[^a-zA-Z 0-9.]+/g, "")
						);
						maybePercentage = false;
					}
					if (value > 0 && maybePercentage) {
						value =
							cell.value && cell.value.indexOf("%") >= 0
								? value * 100
								: value;
					}
					record.push(value);
...
KusaiChartRenderer.prototype.drawRadarChart = function (o) {
	var RadarChart = {
