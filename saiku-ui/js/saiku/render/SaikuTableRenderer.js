function KusaiTableRenderer(data, options) {
	this._data = data;
...
KusaiTableRenderer.prototype.render = function (data, options) {
	var self = this;
	if (data) {
		this._data = data;
	}
	...
	if (
		this._data == null ||
		(this._data.cellset && this._data.cellset.length === 0)
	) {
		return;
	}
	...
	if (this._options.htmlObject) {
		...
		_.defer(function (that) {
			if (
				self._options.hasOwnProperty("batch") &&
				!self._options.hasOwnProperty("batchSize")
			) {
			...
			_.defer(function (that) {
				if (
					self._options.hasOwnProperty("batch") &&
					self._options.hasBatchResult
				) {
					var batchRow = 0;
					var batchIsRunning = false;
					var batchIntervalSize = self._options.hasOwnProperty(
						"batchIntervalSize"
					)
						? self._options.batchIntervalSize
						: 20;
					var batchIntervalTime = self._options.hasOwnProperty(
						"batchIntervalTime"
					)
						? self._options.batchIntervalTime
						: 20;
					...
					var batchInsert = function () {
						...
						if (!batchIsRunning && len > 0 && batchRow < len) {
							...
							for (
								var i = 0;
								batchRow < len && i < batchIntervalSize;
								i++, batchRow++
							) {
								batchContent +=
									self._options.batchResult[batchRow];
							}
							if (batchRow > startb) {
								$(self._options.htmlObject).append(
									$(batchContent)
								);
							}
					...
					var lazyBatchInsert = _.debounce(
						batchInsert,
						batchIntervalTime
					);
					$(self._options.htmlObject)
...
KusaiTableRenderer.prototype.clear = function (data, options) {
	var self = this;
	if (
		this._options &&
		this._options.htmlObject &&
		this._options.hasOwnProperty("batch")
	) {
		$(self._options.htmlObject).parent().parent().unbind("scroll");
	}
};

KusaiTableRenderer.prototype.processData = function (data, options) {
	this._hasProcessed = true;
};

function genTotalDataCells(
	currentIndex,
	cellIndex,
	scanSums,
	scanIndexes,
...
function genTotalHeaderCells(
	...
	for (var i = bottom; i >= 0; i--) {
		if (currentIndex == scanSums[i]) {
			var currentListNode = lists[i][scanIndexes[i]];
			var cssClass;
			if (i == 0 && bottom == 1) cssClass = "col";
			else if (i == bottom) cssClass = "col_total_corner";
			...
			for (var m = 0; m < currentListNode.cells.length; m++) {
				...
				if (bottom == lists.length - 1) {
					...
					if (i == 0 && scanIndexes[i] == 0) {
						if (currentListNode.captions) text += "&nbsp;";
						else text = "";
						text += wrapContent
							? "<span class='i18n'>Grand Total</span>"
							: "Grand Total";
					}
				}
				contents +=
...
function totalIntersectionCells(
	currentIndex,
	bottom,
	scanSums,
	scanIndexes,
...
function isNextTotalsRow(
	...
	for (var i = bottom; i >= 0; i--) {
		if (currentIndex == colScanSums[i]) {
			for (
				var m = 0;
				m < colLists[i][colScanIndexes[i]].cells.length;
				m++
			) {
				contents += "<tr>";
				for (var j = 0; j <= bottom; j++) {
					var cssClass;
					var text = "&nbsp;";
					if (i == 0 && j == 0) cssClass = "row";
					else if (i == j + 1) {
						cssClass = "row_total_corner";
						return j;
					} else if (
						i == j &&
						colLists[i][colScanIndexes[i]].captions
					) {
						cssClass = "row_total_first";
					} else if (i < j + 1) cssClass = "row_total";
					else cssClass = "row_null";
					if (j == bottom) {
...
function genTotalHeaderRowCells(
	currentIndex,
	scanSums,
	scanIndexes,
	totalsLists,
		...
		if (currentIndex == colScanSums[i]) {
			for (
				...
				for (var j = 0; j <= bottom; j++) {
					...
					var text = "&nbsp;";
					if (i == 0 && j == 0) cssClass = "row";
					else if (i == j + 1) cssClass = "row_total_corner";
					else if (
						i == j &&
						colLists[i][colScanIndexes[i]].captions
					) {
				...
				if (totalsLists[ROWS]) {
					for (var z = 0; z < totalsLists[ROWS].length; z++) {
						scanIndexes[z] = 0;
						scanSums[z] =
							totalsLists[ROWS][z][scanIndexes[z]].width;
					}
				}
				...
				for (
					...
					contents +=
						'<td class="data total">' +
						colLists[i][colScanIndexes[i]].cells[m][k].value +
						"</td>";

					if (totalsLists[ROWS]) {
						contents += totalIntersectionCells(
							k + 1,
	...
	for (var dirIndex = 0; dirIndex < dirs.length; dirIndex++) {
		...
		if (totalsLists[dir]) {
			...
			for (var row = 0; row < totalsLists[dir].length; row++) {
				...
				for (
					var totalIndex = 0;
					totalIndex < totalsInfoArray.length;
					totalIndex++
				) {
...
KusaiTableRenderer.prototype.internalRender = function (allData, options) {
	var tableContent = "";
	var rowContent = "";
	...
	if (options) {
		batchSize = options.hasOwnProperty("batchSize")
			? options.batchSize
			: null;
		wrapContent = options.hasOwnProperty("wrapContent")
			? options.wrapContent
			: true;
	}
	var totalsLists = {};
	...
	var hasMeasures =
		allData.query &&
		allData.query.queryModel &&
		allData.query.queryModel.details
			? allData.query.queryModel.details.measures.length
			: 0;

	if (
		typeof this._options.htmlObject === "object" &&
		...
		if (arrColumnTitleTable.length === arrDomColumnTitleTable.length) {
			...
			arrColumnTitleTable = _.intersection(
				arrDomColumnTitleTable,
				arrColumnTitleTable
			);
		}
	}
	...
	for (var row = 0, rowLen = table.length; row < rowLen; row++) {
		...
		if (totalsLists[ROWS]) {
			for (var i = 0; i < totalsLists[ROWS].length; i++) {
				...
				scanSums[ROWS][i] =
					totalsLists[ROWS][i][scanIndexes[ROWS][i]].width;
			}
		}
		...
		if (
			...
			rowContent +=
				'<th class="row_header" style="text-align: right;" colspan="' +
		...
		for (var col = 0, colLen = table[row].length; col < colLen; col++) {
			...
			if (
				...
				if (
					...
					(Settings.ALLOW_AXIS_COLUMN_TITLE_TABLE &&
						allData.query.queryModel.details &&
						allData.query.queryModel.details.location !==
							"BOTTOM") ||
					hasMeasures === 0 ||
				...
				else nextHeader = data[row][col + 1];
				...
				if (isLastColumn) {
					...
					if (header.value == "null") {
						...
						if (totalsLists[ROWS])
							colSpan =
								totalsLists[ROWS][row + 1][
									scanIndexes[ROWS][row + 1]
								].span;
						rowContent +=
							'<th class="col" style="text-align: center;" colspan="' +
							colSpan +
							'" title="' +
							header.value +
							'">' +
					...
					if (
						...
						nextParentsDiffer(data, row, col) ||
						isHeaderLowestLvl ||
						groupChange ||
						maxColspan
					) {
				...
				if (totalsLists[ROWS])
					rowContent += genTotalHeaderCells(
						col - allData.leftOffset + 1,
						row + 1,
						scanSums[ROWS],
						scanIndexes[ROWS],
						totalsLists[ROWS],
				...
				var same =
					!headerSame &&
					!isHeaderLowestLvl &&
					(col == 0 || !topParentsDiffer(data, row, col)) &&
				...
				if (Settings.ALLOW_TABLE_DATA_COLLAPSE) {
					if (row > 0 && row < rowLen - 1) {
						if (
							totalsLists[ROWS] == null ||
							col <= colLen - totalsLists[ROWS].length - 1
						) {
							var checkOther = true;
							if (
								totalsLists[COLUMNS] &&
							...
							if (
								checkOther &&
								nextRow[col].value == header.value
							) {
								if (col > 0) {
									for (var j = 0; j < col; j++) {
										if (
				...
				var value = same
					...
					: '<div rel="' +
					  ...
					  col +
					  '">' +
					  (sameAsPrevValue && Settings.ALLOW_TABLE_DATA_COLLAPSE
							? '<span class="expander expanded" style="cursor: pointer;">&#9660;</span>'
							: "") +
					  header.value +
					  "</div>";
				if (!wrapContent) {
                ...
                */
				...
				if (
					!isHeaderLowestLvl &&
					(typeof nextHeader == "undefined" ||
						nextHeader.value === "null")
				) {
					colspan = 1;
					...
					var groupWidth =
						group in rowGroups
							? rowGroups[group].length -
							  rowGroups[group].indexOf(level)
							: 1;
					for (
						var k = col + 1;
						colspan < groupWidth &&
				...
				batchStarted = true;
				var color = "";
				var val = _.isEmpty(header.value)
					? Settings.EMPTY_VALUE_CHARACTER
					: header.value;
				var arrow = "";

				if (header.properties.hasOwnProperty("image")) {
					var img_height = header.properties.hasOwnProperty(
						"image_height"
					)
						? " height='" + header.properties.image_height + "'"
						: "";
					var img_width = header.properties.hasOwnProperty(
						"image_width"
					)
				...
				if (
					val !== "-" &&
					val !== "" &&
					header.properties.hasOwnProperty("style")
				) {
					color =
				...
				if (header.properties.hasOwnProperty("arrow")) {
					arrow =
						"<img height='10' width='10' style='padding-left: 5px' src='./images/arrow-" +
						header.properties.arrow +
						".gif' border='0'>";
				}
				...
				rowContent +=
					...
					(wrapContent
						? '<div class="datadiv ' +
						  setStyleNegativeNumber(header.properties.raw) +
						  '" alt="' +
						  header.properties.raw +
						  '" rel="' +
						  header.properties.position +
						  '">'
						: "") +
					val +
					arrow +
		...
		if (
			options.hideEmpty &&
			header.type === "DATA_CELL" &&
			rowWithOnlyEmptyCells
		) {
			rowContent = "";
		}
...
KusaiTableRenderer.prototype.renderSummary = function (data) {
	if (data && data.query) {
