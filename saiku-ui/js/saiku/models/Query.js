...
var Query = Backbone.Model.extend({
	...
	initialize: function (args, options) {
		...
		this.model = _.extend({ name: this.uuid }, KusaiOlapQueryTemplate);
		if (args.cube) {
			this.model.cube = args.cube;
		}
		this.helper = new KusaiOlapQueryHelper(this);
	...
	parse: function (response) {
		...
		this.model.properties = _.extend(
			{},
	...
	run: function (force, mdx) {
		...
		if (
			...
			this.model.properties["saiku.olap.query.automatic_execution"] ===
				false &&
		...
		if (exModel.queryType == "OLAP") {
			if (exModel.type == "QUERYMODEL") {
				var columnsOk =
					Object.keys(exModel.queryModel.axes.COLUMNS.hierarchies)
						.length > 0;
				var rowsOk =
					Object.keys(exModel.queryModel.axes.ROWS.hierarchies)
						.length > 0;
				var detailsOk =
					exModel.queryModel.details.axis == "COLUMNS" &&
					exModel.queryModel.details.measures.length > 0;
				...
				if (!rowsOk) {
					errorMessage +=
						'<span class="i18n">You need to include at least one level on rows for a valid query.</span>';
				}
				...
				if (!validated) {
					errorMessage =
						'<span class="i18n">You need to enter some MDX statement to execute.</span>';
				}
			}
		}
		if (!validated) {
