...
var OpenQuery = Backbone.View.extend({
	...
	render: function () {
		...
		Kusai.session.trigger("openQuery:new", { openQuery: this });
	...
	search_file: function (event) {
		var filter = $(this.el)
			...
			.toLowerCase();
		var isEmpty =
			typeof filter == "undefined" || filter === "" || filter === null;
	...
	view_query: function (event) {
		...
		if (
			typeof query.acl != "undefined" &&
			_.indexOf(query.acl, "READ") > -1
		) {
			$(this.el).find(".for_queries .run").parent().removeClass("hide");
		}
		if (
			typeof query.acl != "undefined" &&
			_.indexOf(query.acl, "WRITE") > -1
		) {
			$(this.el)
				.find(".for_queries .delete")
				.parent()
				.removeClass("hide");
		...
		if (
			typeof query.acl != "undefined" &&
			_.indexOf(query.acl, "GRANT") > -1
		) {
			$(this.el)
				.find(".for_queries .edit_permissions")
				.parent()
				.removeClass("hide");
		}
		try {
			...
			if (query_path.length > 1) {
				var folder_path = query_path
					.splice(0, query_path.length - 1)
					.join("/");
				var folder = this.queries[folder_path];
				if (
					typeof folder.acl != "undefined" &&
					_.indexOf(folder.acl, "WRITE") > -1
				) {
		...
		for (var property in query) {
			if (query.hasOwnProperty(property) && property != "name") {
				$properties.append(
					$("<li />").html(
						"<strong>" + property + "</strong> : " + query[property]
					)
				);
			}
		}
	...
	view_folder: function (event) {
		...
		if (
			typeof folder.acl != "undefined" &&
			_.indexOf(folder.acl, "GRANT") > -1
		) {
			$(this.el)
				.find(".for_folder .edit_permissions")
				.parent()
				.removeClass("hide");
		}
	...
	add_folder: function (event) {
		...
		if (typeof $selected !== "undefined" && $selected) {
			if ($selected.hasClass("folder_row")) {
				...
				$selected.hasClass("query") &&
				!$selected.parent().hasClass("RepositoryObjects")
			) {
	...
	fetch_lazyload: function (target, path) {
		var repositoryLazyLoad = new RepositoryLazyLoad(
			{},
			{ dialog: this, folder: target, path: path }
		);
		repositoryLazyLoad.fetch();
		Kusai.ui.block("Loading...");
	},
	...
	open_query: function (viewstate) {
		...
		if (item.fileType === "saiku") {
			...
			Kusai.session.trigger("openQuery:open_query", {
				query: query,
				item: item,
				viewState: state,
			});
	...
	open_contents: function (viewstate) {
		...
		new WarningModal({
			...
			message:
				'<span class="i18n">You are about to open</span> ' +
				files.length +
				' <span class="i18n">queries</span>',
	...
	run_open_contents: function (fileargs) {
		_.forEach(fileargs.files, function (entry) {
			...
			if (
				fileargs.viewstate &&
				!fileargs.viewstate.hasOwnProperty("currentTarget")
			) {
				state = viewstate;
			}
			var tab = Kusai.tabs.add(
	...
	adjust: function () {
		...
		$(this.el)
			.find(".sidebar")
			.css({ width: 350, height: $("body").height() - 87 });
		...
		$(this.el)
			...
			.css({
				...
				height:
					...
					$(this.el).find(".workspace_fields").height() -
					40,
			});
	...
	toggle_sidebar: function () {
		...
		var new_margin = $(this.el).find(".sidebar").hasClass("hide") ? 5 : 265;
