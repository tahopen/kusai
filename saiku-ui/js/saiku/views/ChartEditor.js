...
var ChartEditor = Modal.extend({
		...
		return {
			...
			palette: [
				...
				[
					...
					"rgb(213, 166, 189)",
					"rgb(204, 65, 37)",
					/*'rgb(224, 102, 102)',*/ "rgb(246, 178, 107)",
					"rgb(255, 217, 102)",
					"rgb(147, 196, 125)",
					"rgb(118, 165, 175)",
					"rgb(109, 158, 235)",
	...
	add_color_palette: function (event, color) {
		...
		this.$el
			.find("#" + uid)
			.on("beforeShow.spectrum", this.delete_color_palette);
	...
	delete_color_palette: function function_name(event) {
		...
		if (event.type === "beforeShow") {
			Kusai.inputColorUID = event.target.id;
		}
		...
		if (
			...
			$currentTarget.append(
				'<a href="#delete_color_palette" class="delete-color">x</a>'
			);
		} else {
			$currentTarget.find(".delete-color").remove();
		}
		...
		if (
			event.type === "click" &&
			$(event.target).hasClass("delete-color")
		) {
			$(".sp-container").addClass("sp-hidden");
			$("#" + Kusai.inputColorUID).spectrum("destroy");
			$("#" + Kusai.inputColorUID).remove();
	...
	get_chart_properties: function () {
		...
		if (_.isEmpty(args)) {
			chartOptions = this.workspace.query.getProperty(
			...
			if (
				args[0].chartDefinition &&
				!_.isEmpty(args[0].chartDefinition)
			) {
				chartOptions.chartDefinition = _.isString(
					args[0].chartDefinition
				)
					? JSON.parse(args[0].chartDefinition)
					: args[0].chartDefinition;
			}
		}
		...
		if (
			...
			chartOptions.chartDefinition !== undefined &&
			chartOptions.chartDefinition.title !== null
		) {
			this.$el
				.find("#chart-title")
				.val(chartOptions.chartDefinition.title);
		}

		if (
			chartOptions !== undefined &&
			chartOptions.chartDefinition !== undefined &&
			chartOptions.chartDefinition.baseAxisTitle !== null
		) {
			this.$el
				.find("#xtitle")
				.val(chartOptions.chartDefinition.baseAxisTitle);
		}

		if (
			chartOptions !== undefined &&
			chartOptions.chartDefinition !== undefined &&
			chartOptions.chartDefinition.orthoAxisTitle !== null
		) {
			this.$el
				.find("#ytitle")
				.val(chartOptions.chartDefinition.orthoAxisTitle);
		}

		if (
			chartOptions === undefined ||
			chartOptions.chartDefinition === undefined ||
			chartOptions.chartDefinition.colors === undefined
		) {
			...
			for (var i = 0; i < lenColor; i++) {
				this.add_color_palette(
					null,
					chartOptions.chartDefinition.colors[i]
				);
			}
		}
		...
		if (
			chartOptions !== undefined &&
			chartOptions.chartDefinition !== undefined &&
			chartOptions.chartDefinition.legend !== null
		) {
			this.$el
				.find(
					'input[type=radio][value="' +
						chartOptions.chartDefinition.legend +
						'"]'
				)
				.first()
		...
		if (this.renderer.type === "line") {
			...
			if (
				chartOptions !== undefined &&
				chartOptions.chartDefinition !== undefined &&
				chartOptions.chartDefinition.trend !== null
			) {
				this.$el
					.find("#trend-line")
					.val(chartOptions.chartDefinition.trend.type);
				if (
					chartOptions.chartDefinition.trend.type !== "linear" ||
					chartOptions.chartDefinition.trend.type !== "none"
				) {
					this.$el.find(".div-trend-points").show();
					this.$el
						.find("#trend-points")
						.val(chartOptions.chartDefinition.trend.periods);
				} else {
			...
			if (
				chartOptions !== undefined &&
				chartOptions.chartDefinition !== undefined &&
				chartOptions.chartDefinition.multiChartMax !== null
			) {
				this.$el
					.find("#multiplesunburst-multichartmax")
					.val(chartOptions.chartDefinition.multiChartMax || 30);
			}
			...
			if (
				chartOptions !== undefined &&
				chartOptions.chartDefinition !== undefined &&
				chartOptions.chartDefinition.valuesLabelStyle !== null
			) {
				this.$el
					.find("#pie-values-label-style")
					.val(
						chartOptions.chartDefinition.valuesLabelStyle ||
							"linked"
					);
			}
			...
			if (
				chartOptions !== undefined &&
				chartOptions.chartDefinition !== undefined &&
				chartOptions.chartDefinition.valuesMask !== null
			) {
				this.$el
					.find("#pie-values-mask")
					.val(
						chartOptions.chartDefinition.valuesMask ||
							"{value.percent}"
					);
			}
		} else {
	...
	save: function (event) {
		...
		if (this.$el.find("#chart-title").val() !== null) {
			override.chartDefinition.title = this.$el
				.find("#chart-title")
				.val();
		}

		if (this.$el.find("#xtitle").val() !== null) {
			override.chartDefinition.baseAxisTitle = this.$el
				.find("#xtitle")
				.val();
		}

		if (this.$el.find("#ytitle").val() !== null) {
			override.chartDefinition.orthoAxisTitle = this.$el
				.find("#ytitle")
				.val();
		}

		$.each(
			this.$el.find('.container-colors > input[type="color"]'),
			function (key, value) {
				colors.push(
					self.$el
						.find("#" + $(value).attr("id"))
		...
		override.chartDefinition.legend =
			this.$el.find("input:radio[name=legend]:checked").val() === "true";
		...
		if (this.renderer.type === "multiplesunburst") {
			override.chartDefinition.multiChartMax =
				this.$el.find("#multiplesunburst-multichartmax").val() || 30;
		}

		if (this.renderer.type === "pie") {
			override.chartDefinition.valuesLabelStyle = this.$el
				.find("#pie-values-label-style")
				.val();
			override.chartDefinition.valuesMask =
				this.$el.find("#pie-values-mask").val() || "{value.percent}";
		}

		type = this.workspace.querytoolbar.$el
			.find("ul.chart li")
			.not(".chart_editor")
			.find("a.on")
			.attr("href");
