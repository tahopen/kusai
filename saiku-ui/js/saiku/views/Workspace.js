...
var Workspace = Backbone.View.extend({
	...
	initialize: function (args) {
				...
				{},
				{
					...
					error: function () {
						...
						if ($("body").find(".error_loading_query").length < 1) {
							var message =
								Kusai.i18n &&
								Kusai.i18n.po_file.error_loading_query
									? Kusai.i18n.po_file.error_loading_query
									: null;
							if (!message) {
		...
		Kusai.session.bind("tab:add", this.prepare);
		...
		if (
			args !== undefined &&
			args.processURI !== undefined &&
			args.processURI === false
		) {
			paramsURI = {};
		}
	...
	caption: function (increment) {
		...
		if (increment) {
			Kusai.tabs.queryCount++;
		}
		return (
			"<span class='i18n'>Unsaved query</span> (" +
			Kusai.tabs.queryCount +
			")"
		);
	},
	...
	selected_cube_template: function (selectedCube) {
		var connections = Kusai.session.sessionworkspace;
	...
	template: function () {
		...
		if (this.isUrlCubeNavigation) {
			(selectedCube = this.selected_cube.split("/")[3]),
				(htmlCubeNavigation =
					this.selected_cube_template(selectedCube));
		}

		return _.template(template)({
			cube_navigation: htmlCubeNavigation
				? htmlCubeNavigation
				: Kusai.session.sessionworkspace.cube_navigation,
		});
	},
	...
	refresh: function (e) {
		...
		Kusai.session.sessionworkspace.refresh();
	},
	...
	render: function () {
		...
		Kusai.session.trigger("workspace:new", { workspace: this });

		if (
			Settings.PLUGIN &&
			Settings.BIPLUGIN5 == false &&
			Kusai.session.isAdmin
		) {
			var $link = $("<a />")
				.attr({
					href: "#adminconsole",
				...
				.click(Kusai.AdminConsole.show_admin)
		...
		if (
			!Kusai.session.isAdmin &&
			Settings.SHOW_REFRESH_NONADMIN === false
		) {
		...
		if (
			!Kusai.introDone &&
			Kusai.URLParams.contains({ show_help: paramsURI.show_help })
		) {
			Kusai.intro.start({ fileName: "Workspace" });
			Kusai.introDone = true;
		}
	...
	clear: function () {
		...
		$(this.el)
			...
			.removeClass("on");
		// Trigger clear event
		Kusai.session.trigger("workspace:clear", { workspace: this });
	},
	...
	adjust: function () {
		...
		$(this.querytoolbar.el)
			.find("div")
			.height($("body").height() - heightReduction - 10);

		// Adjust the dimensions of the results window
		var editorHeight = $(this.el).find(".workspace_editor").is(":hidden")
			? 0
			: $(this.el).find(".workspace_editor").height();
		var processingHeight = $(this.el)
			.find(".query_processing")
			.is(":hidden")
			? 0
			: $(this.el).find(".query_processing").height() + 62;
		var upgradeHeight = 0;
		...
		if (this.querytoolbar) {
			$(this.querytoolbar.el)
				.find("a")
				.tipsy({ delayIn: 700, fade: true });
		}
		if (this.toolbar) {
			$(this.toolbar.el).find("a").tipsy({ delayIn: 900, fade: true });
		}
		$(this.el)
	...
	toggle_sidebar: function () {
		...
		var calculatedMargin =
			($(this.el).find(".sidebar").is(":visible")
				? $(this.el).find(".sidebar").width()
				: 0) +
	...
	data_connections: function (paramsURI) {
		var connections = Kusai.session.sessionworkspace.connections;
		var self = this;
					...
					if (schema.cubes.length > 0) {
						_.each(schema.cubes, function (cube) {
							if (
								...
								if (
									paramsURI.schema === schemaName &&
									paramsURI.cube === cubeName
								) {
									self.selected_cube =
										connection.name +
										"/" +
	...
	create_new_query: function (obj) {
		...
		obj.clear();
		obj.processing.hide();
		Kusai.session.trigger("workspace:clear", { workspace: obj });
		...
		obj.metadata = Kusai.session.sessionworkspace.cube[obj.selected_cube];
		var parsed_cube = obj.selected_cube.split("/");
		var cube = parsed_cube[3];
		for (var i = 4, len = parsed_cube.length; i < len; i++) {
			cube += "/" + parsed_cube[i];
		}
		Kusai.events.trigger("workspace:new_query", this, {
			view: this,
			cube: cube,
		});
		...
		if (!this.processedParamsURI) {
			...
			var addCustomFilter = function (axis, filterExpression) {
				...
				if (checkChar.first === "(" && checkChar.last === ")") {
					filterExpression =
						Kusai.trimFirstLastChar(filterExpression);

					if (filterExpression.indexOf("],") !== -1) {
						hierarchyLevel = filterExpression
							.split("],")[0]
							.split("].[");
						level = _.last(hierarchyLevel);
						arrHierarchy = _.first(
							hierarchyLevel,
							hierarchyLevel.length > 2 ? 2 : 1
						);
						filter = filterExpression.split("],")[1].trim();

						for (
							var i = 0, iLen = arrHierarchy.length;
							i < iLen;
							i++
						) {
							hierarchy +=
								"[" +
								Kusai.removeBrackets(arrHierarchy[i]) +
								"].";
						}

						hierarchy = Kusai.trimFirstLastChar(hierarchy, "last");

						this.query.helper.includeLevel(
							axis,
							hierarchy,
							level,
							indexHierarchy
			...
			if (
				Kusai.URLParams.contains({
					default_mdx_filter_rows: paramsURI.default_mdx_filter_rows,
				})
			) {
			...
			if (
				Kusai.URLParams.contains({
					default_mdx_filter_columns:
						paramsURI.default_mdx_filter_columns,
				})
			) {
				addCustomFilter(
					"COLUMNS",
			...
			if (
				Kusai.URLParams.contains({
					default_mdx_filter: paramsURI.default_mdx_filter,
				})
			) {
		...
		if (this.extractDefaultFilters && this.paramsURI) {
		...
		obj.query.save(
			{},
			{ data: { json: JSON.stringify(this.query.model) }, async: false }
		);
		obj.init_query();
	},
	...
	cancel_new_query: function (args) {
		...
		args.$el
			.find('#cubesselect option[value="' + selectedCube + '"]')
			.prop("selected", true);
	},
	...
	init_query: function (isNew) {
		...
		try {
			...
			var properties = this.query.model.properties
				? this.query.model.properties
				: {};

			var renderMode =
				"RENDER_MODE" in Settings
					? Settings.RENDER_MODE
					: "saiku.ui.render.mode" in properties
					? properties["saiku.ui.render.mode"]
					: null;
			var renderType =
				"RENDER_TYPE" in Settings
					? Settings.RENDER_TYPE
					: "saiku.ui.render.type" in properties
					? properties["saiku.ui.render.type"]
					: null;
			...
			if ("chart" == renderMode) {
				...
				this.querytoolbar.$el
					...
					.removeClass("on");
				this.querytoolbar.$el
					.find('ul.chart [href="#' + renderMode + '"]')
					.addClass("on");
			}
		} catch (e) {
			Kusai.error(this.cid, e);
		}
		...
		if (this.query.model.type == "MDX") {
			...
			$(this.toolbar.el)
				.find(
					".auto, .toggle_fields, .query_scenario, .buckets, .non_empty, .swap_axis, .mdx, .switch_to_mdx, .zoom_mode, .drillacross"
				)
				.parent()
				.show();
		...
		if (
			!$(this.el).find(".sidebar").hasClass("hide") &&
			(Settings.MODE == "chart" ||
				Settings.MODE == "table" ||
				Settings.MODE == "map" ||
				Settings.MODE == "view" ||
		...
		if ((Settings.MODE == "view" && this.query) || this.isReadOnly) {
			...
			if (this.selected_cube === undefined) {
				...
				this.selected_cube =
					this.query.model.cube.connection +
					"/" +
		...
		if (this.selected_cube) {
			...
			var cubeModel =
				Kusai.session.sessionworkspace.cube[this.selected_cube];
			...
			$(this.el)
				.find(".metadata_attribute_wrapper")
				.html("")
	...
	set_class_charteditor: function () {
		...
		if (chartOptions) {
			$(this.querytoolbar.el)
				.find('ul.chart [href="#charteditor"]')
				.addClass("on");
		}
	},
	...
	synchronize_query: function () {
		...
		if (
			...
			(!Settings.hasOwnProperty("MODE") ||
				(Settings.MODE != "table" && Settings.MODE != "view"))
		) {
		}
	},
				...
				if (calcMeasures && calcMeasures.length > 0) {
					...
					dimlist
						...
						.draggable({
							...
							connectToSortable: $(self.el).find(
	...
	populate_selections: function (dimlist) {
		...
		if (this.other_dimension) {
			...
			if (axes) {
				for (
					var axis_iter = 0, axis_iter_len = axes.length;
					axis_iter < axis_iter_len;
					axis_iter++
				) {
					...
					if (
						axis.filterCondition !== null ||
						(axis.limitFunction &&
							axis.limitFunction !== null &&
							axis.limitFunction !== "") ||
						axis.sortOrder !== null
					) {
						$axis
							.parent()
							.siblings(".fields_list_header")
							.addClass("on");
					}
					for (
						var dim_iter = 0,
							dim_iter_len = axis.dimensionSelections.length;
						dim_iter < dim_iter_len;
						dim_iter++
					) {
						...
						if (
							...
							var ds =
								Kusai.session.sessionworkspace.cube[
									this.selected_cube
								].get("data").dimensions;
						...
						for (
							var sel_iter = 0,
								sel_iter_len = dimension.selections.length;
							sel_iter < sel_iter_len;
							sel_iter++
						) {
...
*/
								if (
									typeof self.dimension_list != "undefined" &&
									(!$dim.html() || $dim.html() === null)
								) {
								...
								if (type == "dimension") {
									...
									_.each(axes, function (i_axis) {
										if (
											...
											i_axis.sortLiteral.indexOf(
												selection.hierarchyUniqueName
											) != -1
										) {
								...
								if (type == "measure") {
									...
									_.each(axes, function (i_axis) {
										if (
											i_axis.sortLiteral &&
											i_axis.sortLiteral !== null &&
											i_axis.sortLiteral.indexOf(name) !=
												-1
										) {
											$icon.addClass(i_axis.sortOrder);
											sort = true;
	...
	update_parameters: function () {
		...
		if (
			!self.hasOwnProperty("query") ||
			!Settings.ALLOW_PARAMETERS ||
			Settings.MODE === "view" ||
			self.viewState === "view"
		)
			return;
	...
	render_result: function (args) {
		...
		var runtime =
			args.data.runtime !== null
				? (args.data.runtime / 1000).toFixed(2)
				: "";
		/*
		...
		if (Settings.REPORT_TITLES) {
			...
			if (h != undefined) {
				...
				if (headings.title != null && headings.title != "") {
					header =
						'<h3><span class="i18n">Title:</span></h3> &nbsp;' +
						headings.title +
						"<br/>";
				}
				if (headings.variable != null && headings.variable != "") {
					header +=
						'<h3><span class="i18n">Variable:</span></h3> &nbsp;' +
						headings.variable +
						"<br/>";
				}
				if (
					headings.explanation != null &&
					headings.explanation != ""
				) {
					header +=
						'<h3><span class="i18n">Explanation:</span></h3> &nbsp;' +
						headings.explanation;
				}
	...
	switch_view_state: function (mode, dontAnimate) {
		...
		if (target == "edit") {
			...
			$(this.toolbar.el)
				.find(".auto, .toggle_fields, .toggle_sidebar,.switch_to_mdx")
				.parent()
				.hide();
		}
	...
	block: function (message) {
		...
		if (Settings.LOGO_32x32) {
			$(self.el).block({
				message:
					'<img class="saiku_logo_override" style="float:left" src="' +
					Settings.LOGO_32x32 +
					'"/> ' +
					message,
			});
			...
			$(self.el).block({
				message:
					'<span class="saiku_logo" style="float:left">&nbsp;&nbsp;</span> ' +
					message,
			});
			Kusai.i18n.translate();
		}
