...
var Tab = Backbone.View.extend({
	...
	template: function () {
		// Create tab
		return _.template(
			"<a class='saikutab' href='#<%= id %>'><%= caption %></a>" +
	...
	render: function () {
		...
		$.contextMenu({
			...
			callback: function (key, options) {
				...
				var tab = Kusai.tabs.find(selected);
				if (key == "closethis") {
					tab.remove();
					self.select();
					return;
					...
					tab.select();
					Kusai.tabs.close_others(tab);
				} else if (key == "duplicate") {
					Kusai.tabs.duplicate(tab);
				} else if (key == "new") {
					Kusai.tabs.new_tab();
				}
	...
	remove: function (event) {
		if (
			!event ||
			event.which === 2 ||
			$(event.target).hasClass("close_tab")
		) {
