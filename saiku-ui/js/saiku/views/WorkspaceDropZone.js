...
var WorkspaceDropZone = Backbone.View.extend({
	...
	set_measures: function (event, ui) {
		...
		$(this.el)
			...
			.each(function (index, element) {
				...
				Kusai.events.trigger("workspaceDropZone:select_measure", this, {
					measure: measure,
				});
			});
	...
	remove_measure_click: function (event) {
		...
		var target = $(event.target).hasClass("d_measure")
			? $(event.target).find("a")
			: $(event.target);
			...
			if (Settings.ALLOW_PARAMETERS) {
				$.each($(ui.helper).find("li"), function (key, value) {
					if ($(value).hasClass("temphide")) {
						...
						self.workspace.query.helper.removeParameter(
							hierarchy,
							level
						);
					}
	...
	synchronize_query: function () {
		...
		if (
			model.hasOwnProperty("queryModel") &&
			model.queryModel.hasOwnProperty("axes")
		) {
			var axes = model.queryModel.axes;

			for (var axis in axes) {
				var $axis = $(self.el).find(
					'.fields_list[title="' + axis + '"]'
				);
			...
			_.each(measures, function (measure) {
				if (self.workspace.dimension_list != null) {
					var m = $(self.workspace.dimension_list.el)
						.find(
							'.measure_tree a.measure[measure="' +
								measure.name +
								'"]'
						)
						.parent();
	...
	update_dropzones: function () {
		$(this.workspace.el)
			...
			.each(function (idx, ael) {
				...
				if (
					$axis.find(
						"ul.connectable li.selection, ul.connectable li.d_measure"
					).length === 0
				) {
	...
	clear_axis: function (event) {
				...
				for (var lName in axisData.hierarchies[i].levels) {
					if (axisData.hierarchies[i].levels.hasOwnProperty(lName)) {
						if (
							...
							this.workspace.query.helper.removeParameter(
								hierarchy,
								level
							);
						}
		...
		if (
			...
			Kusai.session.trigger("workspaceDropZone:clear_axis", {
	...
	select_dimension: function (event, ui) {
		...
		Kusai.session.trigger("workspaceDropZone:select_dimension", {
			workspace: this.workspace,
		});
		...
		if ($(ui.item).is(":visible")) {
			...
			ui.item
				...
				.each(function (index, element) {
					if (
						$(element).find("ul.hierarchy").attr("hierarchy") ==
						hierarchy
					) {
			...
			if (isCalcMember) {
				...
				if (isNew) {
					...
					self.workspace.query.helper.moveHierarchy(
						fromAxis,
						toAxis,
						hierarchy,
						indexHierarchy
			...
			Kusai.events.trigger("workspaceDropZone:select_dimension", this, {
				level: level,
	...
	find_type_time: function (dimension, hierarchy, level) {
		...
		var metadata = this.workspace.metadata.attributes.data,
			value = {};
		value.dimensions = _.findWhere(metadata.dimensions, {
			name: dimension,
		});
		if (hierarchy === undefined) {
			hierarchy = dimension;
		}
		value.hierarchies = _.findWhere(value.dimensions.hierarchies, {
			name: hierarchy,
		});
		...
		if (value.level === null || value.level === undefined) {
			value.level = _.findWhere(value.hierarchies.levels, {
				caption: level,
			});
		}
		return value;
	...
	selections: function (event, ui) {
		...
		if (
			...
			(objData.level.annotations.AnalyzerDateFormat !== undefined ||
				objData.level.annotations.KusaiDayFormatString !== undefined) &&
			((_.has(memberLevel, "selection") &&
				memberLevel.selection.members.length === 0) ||
				(_.size(memberLevel) === 1 && _.has(memberLevel, "name")) ||
				(_.has(memberLevel, "mdx") && memberLevel.mdx) ||
				(_.size(memberLevel) === 2 &&
					_.has(memberLevel, "name") &&
					_.has(memberLevel, "mdx")))
		) {
			...
			new DateFilterModal({
				...
				data: objData,
				analyzerDateFormat:
					objData.level.annotations.AnalyzerDateFormat,
				dimHier: dimHier,
				key: key,
				workspace: this.workspace,
			}).open();
	...
	measure_action: function (event) {
		...
		var $target = $(event.target).hasClass("limit")
			? $(event.target)
			: $(event.target).parent();
				...
				return {
					callback: function (key, options) {
						if (key === "cancel") {
							...
							details.location =
								KusaiOlapQueryTemplate.queryModel.details.location;
							details.axis =
								KusaiOlapQueryTemplate.queryModel.details.axis;
						} else {
	...
	limit_axis: function (event) {
		...
		$.contextMenu({
			...
			build: function ($trigger, e) {
				...
				var items = {};
				var measures =
					Kusai.session.sessionworkspace.cube[cube].get(
						"data"
					).measures;
					...
					sort: {
						...
						items: {
							ASCQuick: {
								name: "Ascending",
								i18n: true,
								items: addFun(items, "ASC"),
							},
							DESCQuick: {
								name: "Descending",
								i18n: true,
								items: addFun(items, "DESC"),
							},
							BASCQuick: {
								name: "Ascending (Breaking Hierarchy)",
								i18n: true,
								items: addFun(items, "BASC"),
							},
							BDESCQuick: {
								name: "Descending (Breaking Hierarchy)",
								i18n: true,
								items: addFun(items, "BDESC"),
							},
				...
				var selectedMeasures =
					query.helper.model().queryModel.details.measures;
				_.each(selectedMeasures, function (measure) {
					var foldName =
						"fold_" +
						measure.name.replace(/\s/g, "_").toLowerCase();
					...
					fold.items["show_totals_nil_" + measure.name] = {
						name: self.formatAggregatorName(
							"None",
							"nil",
							a,
							measure
				...
				if (totalFunction) {
					for (var key in totalItems) {
						if (
							key.substring("show_totals_".length) ==
							totalFunction
						) {
							totalItems[key].name =
								"<b>" + totalItems[key].name + "</b";
						}
					}
				} else {
					totalItems.show_totals_not.name =
						"<b>" + totalItems.show_totals_not.name + "</b";
				}
				...
				if (isFilter) {
					var f = citems.filter;
					f.name = "<b>" + f.name + "</b>";
					f.items.customfilter.name =
						"<b>" + f.items.customfilter.name + "</b>";
				}
				...
				return {
					callback: function (key, options) {
						...
						if (key == "clearfilter") {
							...
							self.workspace.query.helper.removeFilter(
								a,
								"Generic"
							);
							...
							if (tokens.length > 1) {
								...
								total = tokens[0];
								var metric = key.substring(
									("show_totals_" + total).length + 1
								);
							...
							if (
								...
								a.sortOrder = fun;
								a.sortEvaluationLiteral =
									items[ikey].payload.sortliteral;
