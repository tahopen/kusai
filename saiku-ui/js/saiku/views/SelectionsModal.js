...
var SelectionsModal = Modal.extend({
	...
	events: {
		...
		"dblclick .selection_options li.option_value label":
			"click_move_selection",
	...
	initialize: function (args) {
		...
		_.extend(this, args);
		this.options.title =
			"<span class='i18n'>Selections for</span> " + this.name;
		...
		if (
			(this.source === "DateFilterModal" &&
				_.has(level, "selection") &&
				level.selection.members.length === 0) ||
			(this.source === "DateFilterModal" &&
				_.size(level) === 1 &&
				_.has(level, "name"))
		) {
		...
		if (Settings.ALLOW_PARAMETERS) {
			if (level) {
				var pName = level.selection
					? level.selection.parameterName
					: null;
				if (pName) {
					$(this.el).find("#parameter").val(pName);

					if (
						this.query.helper.model().parameters[pName] != undefined
					) {
						this.paramvalue = this.query.helper
							.model()
							.parameters[pName].split(",");
					}
				}
			}
	...
	init_totals: function (hierarchy, level) {
		...
		var measuresArray =
			self.workspace.query.model.queryModel.details.measures;
		for (var j = 0; j < measuresArray.length; j++) {
			$(this.el)
				.find("#div-totals-container")
				.append(
					_.template($("#template-selections-totals").html())({
						measure: measuresArray[j],
					})
				);
		}
		...
		if (this.show_totals_option.length > 0) {
			if (this.per_metrics_totals) {
				$(this.el)
					...
					.each(function (index) {
						if (
							typeof self.show_totals_option[index] !==
							"undefined"
						) {
	...
	get_members: function () {
		...
		var message =
			'<span class="processing_image">&nbsp;&nbsp;</span> <span class="i18n">' +
			self.message +
			"</span> ";
	...
	search_members: function () {
		...
		self.workspace.query.action.gett(self.search_path, {
			...
			data: {
				search: search_term,
				searchlimit: self.members_search_limit,
			},
		});
	},
	...
	get_calcmembers: function () {
		...
		if (this.topLevel.name === this.member.level) {
			_.filter(calcMembers, function (value) {
				if (
					value.hierarchyName === hName &&
					_.isEmpty(value.parentMember)
				) {
					value.uniqueName =
						value.hierarchyName + ".[" + value.name + "]";
			...
			_.filter(calcMembers, function (value) {
				if (
					value.hierarchyName === hName &&
					value.parentMemberLevel === self.member.level
				) {
					value.uniqueName =
						value.parentMember + ".[" + value.name + "]";
					arrCalcMembers.push(value);
				}
			});
		}
	...
	populate: function (model, response) {
		...
		this.members_search_server =
			this.available_members.length >= this.members_limit ||
			this.available_members.length == 0;
		...
		if (this.members_search_server) {
			$(this.el)
				.find(".warning")
				.text(
					"More items available than listed. Pre-Filter on server."
				);
		} else {
		...
		if (hierarchy && hierarchy.levels.hasOwnProperty(lName)) {
			...
			if (hierarchy.levels[lName].selection) {
				_.each(hierarchy.levels[lName].selection.members, function (f) {
					var cmem =
						self.workspace.query.helper.getCalculatedMembers();
					var calc = false;
			...
			this.selection_type = hierarchy.levels[lName].selection
				? hierarchy.levels[lName].selection.type
				: "INCLUSION";
		}
		...
		if (
			$(this.el).find(
				".used_selections .selection_options li.option_value"
			).length == 0
		) {
			var selectedMembers = $(this.el).find(
				".used_selections .selection_options"
			);
			selectedMembers.empty();
			var selectedHtml = _.template(
				$("#template-selections-options").html()
		...
		this.available_members = _.select(this.available_members, function (o) {
			return (
				used_members.indexOf(o.obj ? o.obj.caption : o.caption) === -1
			);
		});

		if (this.available_members.length > 0) {
			var availableMembersSelect = $(this.el).find(
				".available_selections .selection_options"
			);
		...
		$(this.el)
			...
			.autocomplete({
				...
				source: function (request, response) {
					...
					var search_target =
						self.show_unique_option == false ? "caption" : "name";
				...
				select: function (event, ui) {
					...
					$(self.el)
						.find(
							'.available_selections .selection_options input[value="' +
					...
					$(self.el)
						.find(
							'.used_selections .selection_options input[value="' +
								encodeURIComponent(searchVal) +
								'"]'
						)
						.parent()
					...
					$(option).appendTo(
						$(self.el).find(
							".used_selections .selection_options ul"
						)
					);
	...
	move_selection: function (event) {
		...
		var $to =
			action.indexOf("add") !== -1
				...
				: $(this.el).find(
						".available_selections .selection_options ul"
				  );
		var $from =
			action.indexOf("add") !== -1
	...
	save: function () {
		...
		if (hierarchy && hierarchy.levels.hasOwnProperty(lName)) {
			...
			hierarchy.levels[lName].selection = {
				type: selectionType,
				members: updates,
			};
			if (Settings.ALLOW_PARAMETERS && parameterName) {
				hierarchy.levels[lName].selection["parameterName"] =
					parameterName;
