...
var OpenDialog = Modal.extend({
	...
	initialize: function (args) {
		...
		this.bind("open", function () {
			...
			var perc =
				((($("body").height() - 600) / 2) * 100) / $("body").height();
			$(this.el).find(".RepositoryObjects").height(height);
			$(this.el).dialog("option", "position", "center");
			$(this.el)
				.parents(".ui-dialog")
				.css({ width: "550px", top: perc + "%" });
			$(this.el)
				.find(".dialog_footer")
				.find('a[href="#open_query"]')
				.hide();
	...
	context_menu_disabled: function () {
		this.$el
			.find(".RepositoryObjects")
			.find(".folder_row, .query")
			.addClass("context-menu-disabled");
	},
	...
	export_zip: function (event) {
		...
		if (typeof file != "undefined" && file !== "") {
			var url =
				Settings.REST_URL +
				new RepositoryZipExport({ directory: file }).url();
	...
	select_folder: function () {
		...
		var file =
			foldersSelected.length > 0
				? foldersSelected.children("a").attr("href").replace("#", "")
				: null;
		if (typeof file != "undefined" && file !== null && file !== "") {
			var form = $("#importForm");
			form.find(".directory").val(file);
			var url =
				Settings.REST_URL + new RepositoryZipExport().url() + "upload";
	...
	open_query: function (event) {
		...
		var tab = Kusai.tabs.add(
			new Workspace({ query: query, item: item, processURI: false })
		);
	...
	select_last_location: function () {
		if (localStorage.getItem("last-folder") && !Settings.REPOSITORY_LAZY) {
			var p = $(this.el).find(
				'a[href="\\#' + localStorage.getItem("last-folder") + '"]'
			);
