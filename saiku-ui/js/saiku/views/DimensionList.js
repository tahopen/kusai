...
var DimensionList = Backbone.View.extend({
	...
	render: function () {
		...
		if (this.cube && this.cube.has("template_attributes")) {
			...
			$(this.el).html(
				"Could not load attributes. Please log out and log in again."
			);
		} else {
	...
	select: function (event) {
		...
		if ($target.hasClass("root")) {
			...
			if ($target.hasClass("expand")) {
				Kusai.events.trigger("workspace:expandDimension", this, null);
			}
		}
	...
	select_dimension: function (event, ui) {
		...
		if (
			$(this.workspace.el).find(
				".workspace_fields ul.hierarchy[hierarchy='" + hierarchy + "']"
			).length > 0
		) {
			var $level = $(this.workspace.el)
				.find(
					".workspace_fields ul[hierarchy='" +
						hierarchy +
						"'] a[level='" +
						level +
						"']"
				...
				.parent()
				.show();
			axisName = $level.parents(".fields_list_body").hasClass("rows")
				? "ROWS"
				: "COLUMNS";
		} else {
			var $axis =
				$(this.workspace.el).find(
					".workspace_fields .fields_list[title='ROWS'] ul.hierarchy"
				).length > 0
					? $(this.workspace.el).find(
							".workspace_fields .fields_list[title='COLUMNS'] ul.connectable"
					  )
					: $(this.workspace.el).find(
							".workspace_fields .fields_list[title='ROWS'] ul.connectable"
					  );
		...
		if (isCalcMember) {
			...
			this.workspace.query.helper.includeLevelCalculatedMember(
				axisName,
				hierarchy,
				level,
				uniqueName
		...
		Kusai.session.trigger("dimensionList:select_dimension", {
