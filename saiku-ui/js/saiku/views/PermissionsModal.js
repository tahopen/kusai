...
var PermissionsModal = Modal.extend({
	...
	initialize: function (args) {
		...
		this.bind("open", Kusai.i18n.translate());
		this.render();
		// Load template
		$(this.el)
			.find(".dialog_body")
...
*/
		...
		var definedRoles =
			typeof acl.get("roles") == "undefined" || acl.get("roles") === null
				? {}
				: acl.get("roles");
		this.rolesacl = definedRoles;
		var templ_roles = _.template(
			$("#template-permissions-rolelist").html()
		)({ roles: definedRoles });

		$(this.el).find(".rolelist").html(templ_roles);

		var owner =
			typeof acl.get("owner") == "undefined" || acl.get("owner") === null
				? ""
				: acl.get("owner");
		var atype =
			typeof acl.get("type") == "undefined" || acl.get("type") === null
				? null
				: acl.get("type");
			...
			if (rolecount > 0) {
				...
				$(
					"<option value='" +
						...
						aclstring +
						"]</option>"
				).appendTo($(this.el).find(".select_roles"));
	...
	keep_private: function (event) {
		...
		if (isPrivate) {
			...
			$(
				"input.acl, input.filterbox, input.add_role, input.remove_acl"
			).prop("disabled", true);
	...
	remove_acl: function (event) {
		...
		$(this.el)
			.find(".select_roles option:selected")
			.each(function (index) {
	...
	ok: function () {
		...
		var acl = {};
		if (this.acltype == "PRIVATE") {
			acl = { type: "PRIVATE", owner: Kusai.session.username };
		} else {
			acl = {
				type: "SECURED",
				roles: this.rolesacl,
				owner: Kusai.session.username,
			};
		}
