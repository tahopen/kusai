...
var SaveQuery = Modal.extend({
	...
	set_name: function (folder, file) {
		if (folder !== null) {
			...
			name =
				(this.folder_name !== null ? this.folder_name + "/" : "") +
	...
	save: function (event) {
		...
		var foldername =
			""; /* XXX == root, should it be something different than ''? */
		/*
			...
			if (name !== null && name.length > 0) {
				this.repository.fetch({
					success: function (collection, response) {
						...
						if (
							paths.indexOf(name) > -1 &&
							self.query.get("name") !== name
						) {
							new OverwriteModal({
								name: name,
	...
	copy_to_repository: function () {
		...
		// Rename tab
		this.query.workspace.tab.$el
			.find(".saikutab")
			.text(file.replace(/^.*[\\\/]/, "").split(".")[0]);
