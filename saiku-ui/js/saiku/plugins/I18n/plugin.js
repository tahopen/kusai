...
Kusai.i18n = {
	...
	translate: function (specificElement) {
		if (specificElement) {
			$(specificElement).find(".i18n").i18n(Kusai.i18n.po_file);
		} else {
			$(".i18n").i18n(Kusai.i18n.po_file);
		}
	},
	automatic_i18n: function () {
		...
		// compatible 'zh-CN' -> 'zh';
		if (Kusai.i18n.locale == "zh") {
			Kusai.i18n.locale = "cn";
		}
		// Load language file if edit in Settings.js
		else if (Settings.I18N_LOCALE !== "en") {
			Kusai.i18n.locale = Settings.I18N_LOCALE;
		}
		// Load language file if add a parameter `lang` in URL.
		// for example: ?lang=cn
		else if (_.has(paramsURI, "lang")) {
			Kusai.i18n.locale = paramsURI["lang"];
		}

		if (Kusai.i18n.locale != "en") {
			$.ajax({
				url: "js/saiku/plugins/I18n/po/" + Kusai.i18n.locale + ".json",
				type: "GET",
				dataType: "json",
				success: function (data) {
					Kusai.i18n.po_file = data;
	...
	improve_translation: function () {
		Kusai.tabs.add(new TranslationTab());
		return false;
	},
};
...
function recursive_menu_translate(object, po_file) {
	if (object && object.hasOwnProperty("name") && object.i18n && po_file) {
		...
		if (typeof translation != "undefined") {
			...
			if (Kusai.i18n.elements.indexOf(object.name) === -1) {
				Kusai.i18n.elements.push(object.name);
			}
		}
	}
	...
	$.fn.i18n = function (po_file) {
		...
		return $.each(this, function () {
			...
			if (element.html()) {
				...
				if (
					Kusai.i18n.elements.indexOf &&
					Kusai.i18n.elements.indexOf(element.html()) === -1
				) {
					Kusai.i18n.elements.push(element.html());
				}
				if (translated_text) {
			...
			if (element.attr("title")) {
				...
				if (
					Kusai.i18n.elements.indexOf &&
					Kusai.i18n.elements.indexOf(element.attr("title")) === -1
				) {
					Kusai.i18n.elements.push(element.attr("title"));
				}
				if (translated_title) {
			...
			if (element.attr("original-title")) {
				...
				if (
					...
					Kusai.i18n.elements.push(element.attr("original-title"));
				}
				if (translated_title) {
			...
			if (
				element.attr("value") &&
				element.prop("tagName").toLowerCase() !== "option"
			) {
				// console.log("value:" + element.attr('value'));
				translated_value = translate(element.attr("value"), po_file);
				if (
					Kusai.i18n.elements.indexOf &&
					Kusai.i18n.elements.indexOf(element.attr("value")) === -1
				) {
					Kusai.i18n.elements.push(element.attr("value"));
				}
				if (translated_value) {
...
var TranslationTab = Backbone.View.extend({
	...
	caption: function () {
		return Kusai.i18n.po_file["Improve this translation"]
			? Kusai.i18n.po_file["Improve this translation"]
	...
	render: function () {
		...
		for (var i = 0, len = Kusai.i18n.elements.length; i < len; i++) {
			translation_table[Kusai.i18n.elements[i]] = {
				value: Kusai.i18n.po_file[Kusai.i18n.elements[i]],
				name: encodeURI(Kusai.i18n.elements[i]),
			};
		}
		var table = _.template(
	...
	submit: function () {
		var translation = { locale: Kusai.i18n.locale };
		$(this.el)
			.find(".changed")
			.each(function (element) {
				translation[decodeURI($(this).attr("name"))] = encodeURI(
					$(this).val()
				);
			});
		window.location =
			"mailto:contact@analytical-labs.com?subject=Translation for " +
			Kusai.i18n.locale +
			"&body=" +
			JSON.stringify(translation);
		//Translate.log(translation);
		Kusai.ui.block("Thank you for improving our translation!");
		this.tab.remove();
		_.delay(function () {
			Kusai.ui.unblock();
		}, 1000);
...
Kusai.events.bind("session:new", function () {
	...
	Kusai.session.bind("tab:add", Kusai.i18n.translate);

	/**
	 * Add translate button
	 */
	if (Kusai.i18n.locale != "en" && Kusai.session.isAdmin) {
		var $link = $("<a />")
			.text(Kusai.i18n.locale)
			.attr({
				href: "#translate",
				title: "Improve this translation",
			})
			.click(Kusai.i18n.improve_translation)
