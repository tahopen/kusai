...
var Kusai = {
	 ...
	 *
	 *    if (Kusai.URLParams.contains({ splash: paramsURI.splash })) {
	 *        // Do something...
	 *    }
	 */
	URLParams: {
		...
		paramsURI: function () {
			...
			if (window.location.search.length > 1) {
				for (keyId = 0; keyId < lenCouples; keyId++) {
					...
					paramsURI[decodeURIComponent(keyValue[0])] =
						keyValue.length > 1
							? // ? this.buildValue(decodeURIComponent(keyValue[1]))
							  this.buildValue(
									decodeURIComponent(
										this.joinArrayValues(keyValue)
									)
							  )
							: null;
				}
	 ...
	 * @example:
	 *
	 *    Kusai.loadCSS('PATH_OF_YOUR_CSS');
	 */
	 ...
	 * @example:
	 *
	 *    Kusai.toPattern(1099911111, '(99) 9999-9999');  // -> (10) 9991-1111
	 *    Kusai.toPattern(12122000, '99/99/9999');        // -> 12/12/2000
	 *    Kusai.toPattern(99911111101, '999.999.999-99'); // -> 999.111.111-01
	 *    Kusai.toPattern('ABC1234', 'AAA-9999');         // -> ABC-1234
	 */
	 ...
	 * @example:
	 *
	 *    Kusai.replaceString('World', 'Web', 'Kusai Analytics World'); // -> Kusai Analytics Web
	 */
	 ...
	 * @example:
	 *
	 *    Kusai.removeBrackets('[Time].[Time].[Year]'); // -> Time.Time.Year
	 */
	 ...
	 * @example:
	 *
	 *    Kusai.trimFirstLastChar('Kusai Analytics', 'first'); // -> aiku Analytics
	 *    Kusai.trimFirstLastChar('Kusai Analytics', 'last');  // -> Kusai Analytic
	 *    Kusai.trimFirstLastChar('Kusai Analytics');          // -> aiku Analytic
	 */
...
/**
 * Kusai Singleton pattern
 */
Kusai.singleton = (function () {
	"use strict";
				...
				settingsoverride.fetch({
					success: function () {
						...
						plugins.each(function (log) {
							...
							if (
								log.attributes.path !==
								"js/saiku/plugins/I18n/plugin.js"
							) {
								jQuery.ajax({
									async: false,
									type: "GET",
									...
									success: function () {
										if (j === i) {
											...
											settingsoverride.each(function (
												log
											) {
												l = l + 1;
								...
								if (j === i) {
									...
									settingsoverride.each(function (log) {
										...
										if (Settings.CSS !== undefined) {
											Kusai.loadCSS(Settings.CSS, null);
										}
...
var KusaiTimeLogger = function (element) {
	this._element = $(element);
	this._timestamps = [];
	this._events = [];
};

KusaiTimeLogger.prototype.log = function (eventname) {
