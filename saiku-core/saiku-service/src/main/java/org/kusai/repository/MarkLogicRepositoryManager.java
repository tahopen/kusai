...
public class MarkLogicRepositoryManager implements IRepositoryManager {
  ...
  public MarkLogicRepositoryManager(String host, int port, String username, String password, String database,
      String data, ScopedRepo sessionRegistry, boolean workspaces) {
    this.host = host;
    this.port = port;
  ...
  public static synchronized MarkLogicRepositoryManager getMarkLogicRepositoryManager(String host, int port,
      String username, String password, String database, String data, ScopedRepo sessionRegistry, boolean workspaces) {
    if (instance == null) {
      instance = new MarkLogicRepositoryManager(host, port, username, password, database, data, sessionRegistry,
          workspaces);
    }
  ...
  public boolean moveFolder(String user, String folder, String source, String target) throws RepositoryException {
    /*
     * TODO - MarkLogic doest not support moving directories or documents, instead
     * you should create another directory
     * and document with the same content, with another name, and delete the old
     * directory/document.
     */
    return false;
  }
  ...
  private void executeUpdate(String update, Map<String, String> parameters, String errorMsg)
      throws RepositoryException {
    Session session = createUpdateSession();
