 ...
 */
package org.kusai.olap.util.formatter;
...
import org.kusai.olap.dto.resultset.Matrix;
...
public class FlattenedCellSetFormatter implements ICellSetFormatter {
  ...
  private Matrix formatPage(final CellSet cellSet, final int[] pageCoords, final CellSetAxis columnsAxis,
    ...
    // if (matrix.get(xx,yy-1) != null && matrix.get(xx,yy) != null &&
    // matrix.get(xx,yy-1).getRawValue() != null
    // && matrix.get(xx,yy-1).getRawValue().equals(matrix.get(xx,
    // yy).getRawValue()))
    // {
