var KusaiTableRenderer = _.extend(KusaiRenderer, {
  key: "table",
});

KusaiTableRenderer.prototype._render = function (data, options) {
  var self = this;
  if (data) {
    this._data = data;
  }
...
KusaiTableRenderer.prototype._processData = function (data, options) {
        ...
        for (var j = 0; j <= bottom; j++) {
          ...
          if (j == bottom) {
            ...
            if (i == 0 && colScanIndexes[i] == 0) {
              if (colLists[i][colScanIndexes[i]].captions) text += "&nbsp;";
              else text = "";
              text += wrapContent
                ? "<span class='i18n'>Grand Total</span>"
                : "Grand Total";
      ...
      if (
        ...
        if (isLastColumn) {
          ...
          if (
            ...
            if (header.value == "null") {
              rowContent +=
                '<th class="col_null" colspan="' + colSpan + '">&nbsp;</th>';
            } else {
              if (totalsLists[ROWS])
                colSpan =
                  totalsLists[ROWS][row + 1][scanIndexes[ROWS][row + 1]].span;
        ...
        if (header.properties.hasOwnProperty("image")) {
          var img_height = header.properties.hasOwnProperty("image_height")
            ? " height='" + header.properties.image_height + "'"
            : "";
          var img_width = header.properties.hasOwnProperty("image_width")
