Kusai.AdminConsole = {
	show_admin: function () {
		...
		if (tab) {
			...
			Kusai.tabs.add(new AdminConsole());
		}
...
var AdminConsole = Backbone.View.extend({
	...
	show_license_info: function (event) {
		...
		$(this.el)
			.find(".license_type > li:nth-child(1)")
			.append(this.licenseInfo.licenseType);
		$(this.el)
			.find(".license_type > li:nth-child(2)")
			.append(yourDate.toLocaleDateString());
		$(this.el)
			.find(".license_type > li:nth-child(3)")
			.append(this.licenseInfo.name);
		$(this.el)
			.find(".license_type > li:nth-child(4)")
			.append(this.licenseInfo.hostname);
		$(this.el)
			.find(".license_type > li:nth-child(5)")
			.append(this.licenseInfo.userLimit);
	},
	...
	trigger_set_jdbcdriver: function (event) {
		...
		var $currentTarget = $(event.currentTarget);
		var regexp =
			/jdbc:mysql:|jdbc:postgresql:|jdbc:oracle:|jdbc:drill:|jdbc:jtds:sqlserver:/i;
	...
	change_driver: function () {
		...
		Kusai.events.trigger("admin:changedriver", {
			div: div,
			type: type,
		});
	},
	...
	create_schema: function (event) {
		...
		$(this.el).find(".user_info").html(html);
		Kusai.events.trigger("admin:viewdatasource", {
			admin: this,
		});
	},
	...
	render: function () {
		...
		$.each(menuitems, function (key, item) {
			recursive_menu_translate(item, Kusai.i18n.po_file);
		});
		...
		$.contextMenu({
			...
			events: {
				show: function (opt) {
					...
					if (
						typeof item.acl != "undefined" &&
						_.indexOf(item.acl, "WRITE") < 0
					) {
			...
			callback: function (key, options) {
				var path = $(this).find("a").attr("href").replace("#", "");
				var item = self.queries[path];
				self.selected_query = new SavedQuery({
					file: path,
					name: item.name,
					type: item.type,
				});
	...
	backup_restore_template: _.template(
		"<div><h1>Backup</h1><p><a href='<%= Settings.REST_URL + AdminUrl %>/backup' class='form_button'>Backup Now!</a></p>" +
			"<hr>" +
			"<h1>Restore</h1>" +
	...
	usertemplate: _.template(
		" <form><div id='accordion'><h3 class='accordion-toggle' >User Details</h3>" +
	...
	schemauploadtemplate: _.template(
	...
	licenseInfoTemplate: _.template(
	...
	licenseAddUserTemplate: _.template(
		"<form>" +
			"<h3>Add user</h3><br>" +
	...
	view_schema: function (event) {
		...
		$(this.el).find(".user_info").html(html);
		Kusai.events.trigger("admin:viewschema", {
			admin: this,
		});
	...
	view_datasource: function (event) {
		...
		var s = this.schemas;
		var html = this.datasourcetemplate({
			conn: user.attributes,
			schemas: s.models,
			properties: this.pkeys,
		});
		...
		if (user.get("security_type")) {
			$(this.el)
				.find("#secselect")
				.val(user.get("security_type").toUpperCase());
		}
	...
	save_user: function (event) {
		...
		if (username === null) {
			$.notify("Cannot update user with empty username", {
	...
	save_new_user: function (event) {
		...
		if ($newtarget.val() == $newtarget2.val()) {
			user.set({
				username: username.val(),
				email: emailaddress.val(),
				roles: that.temproles,
				password: $newtarget.val(),
			});
			this.users.add(user);
			user.save(
				{},
	...
	clear_users: function () {
		...
		$(this.el)
			.find(".inner_users")
			.append("<li class='create_user'>Add User</li>");
	},
	clear_datasources: function () {
		$(this.el).find(".inner_datasource").empty();
		Kusai.events.trigger("admin:loaddatasources", {
			admin: this,
		});
	...
	clear_schema: function () {
		...
		Kusai.events.trigger("admin:loadschema", {
			admin: this,
		});
	...
	uploadFile: function (event) {
		...
		this.schemas.create(
			{
				file: file,
				name: $(this.el).find("input[name='schemaname']").val(),
			},
			{
				processData: true,
				success: function () {
				...
				error: function (data, xhr) {
					$(that.el)
						.find("#uploadstatus")
						.html("Upload failed!<br/>(" + xhr.responseText + ")");
					that.schemas.fetch();
				},
			}
		);
	...
	save_datasource: function (event) {
		...
		var isVisibleAdvancedConn = this.$el
			.find(".advconnection")
			.is(":visible");
		var alertMsg = "";
		var conn;
		...
		if (advanced !== null && advanced !== undefined && advanced !== "") {
			...
			var dataSource =
				...
				"location=jdbc:mondrian:Jdbc=jdbc:calcite:model=mongo:///etc/mongoschema/" +
				mongoSchema +
				";Catalog=mondrian://" +
				mondrianSchema +
				";JdbcDrivers=org.apache.calcite.jdbc.Driver;\n" +
				"username=admin\n" +
				"password=admin";
			...
			if (this.$el.find(".extpropselect").val()) {
				conn.set({
					propertyKey: this.$el.find(".extpropselect").val(),
				});
			}
		}
				...
				{},
				{
					...
					error: function (data, xhr) {
						self.$el
							.find("#savestatus")
							.html(
								"Save failed!<br/>(" + xhr.responseText + ")"
							);
						self.schemas.fetch();
					},
				}
	...
	get_data_sources: function (event) {
		...
		new DataSourcesModal({ dialog: this, formElements: formElements })
			.render()
			.open();
	},
...
Kusai.events.bind("admin:loaddatasources", function (admin) {
	$(admin.admin.el)
		.find(".inner_datasource")
		.append("<li class='i18n create_datasource'>Add Data Source</li>");
});
Kusai.events.bind("admin:loadschema", function (admin) {
	$(admin.admin.el)
		.find(".inner_schema")
		.append("<li class='i18n create_schema'>Add Schema</li>");
});
...
Kusai.events.bind("admin:changedriver", function (options) {
	var div = options.div;
