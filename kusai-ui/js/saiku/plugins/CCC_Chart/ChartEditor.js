var ChartEditor = Backbone.View.extend({
	...
	templateEditor:
		...
		'</ul><span class="logo">' +
		'<a class="saiku sprite" title="Kusai - Next Generation Open Source Analytics" href="http://www.analytical-labs.com/"></a>' +
	...
	initialize: function (args) {
		...
		$(this.el).html(
			_.template(this.templateEditor)({ chartList: chartList })
		);

		$(this.el).find(".sidebar").css({ width: "400" });
		this.chartId = _.uniqueId("chartobject_");
		$(this.el)
			.find(".chartworkspace_inner")
			.html("<div id='" + this.chartId + "'></div>");
	...
	render_chart_properties: function (chartName, properties) {
		...
		options = _.sortBy(options, function (property) {
			return property.Order;
		});

		var table =
			"<table class='propertiesviewer' style='border: 1px solid grey'>";
		_.each(options, function (property) {
			var value = property.DefaultValue;
			if (typeof properties !== "undefined") {
				value = self.chartDefinition[property.Name]
					? self.chartDefinition[property.Name]
					: "";
			}

			table +=
				"<tr>" +
				"<td class='data property' title='" +
				property.Tooltip +
				"' href='#" +
				property.Name +
				"'>" +
	...
	click_property: function (event) {
		$target = $(event.target).hasClass("value")
			? $(event.target)
			: $(event.target).parent().find(".value");
	...
	render_chart: function () {
		...
		if (
			options["width"] &&
			max_width > 0 &&
			(options.width <= 0 || options.width > max_width)
		) {
		...
		if (
			options["height"] &&
			max_height > 0 &&
			(options.height <= 0 || options.height > max_height)
		) {
		...
		try {
			...
			$(this.el)
				.find("#" + this.chartId)
				.text("Could not render chart<br>" + e);
		}
	},
