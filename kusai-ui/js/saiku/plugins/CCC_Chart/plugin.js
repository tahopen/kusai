...
var Chart = Backbone.View.extend({
	...
	initialize: function (args) {
		...
		this.renderer = new KusaiChartRenderer(null, {
			htmlObject: $(this.el),
			zoom: true,
			adjustSizeTo: ".workspace_results",
		...
		this.workspace.bind("query:run", function () {
			if (
				!$(self.workspace.querytoolbar.el)
					.find(".render_chart")
					.hasClass("on")
			) {
				return false;
		...
		var pseudoForm =
			"<div id='nav" +
			this.id +
			"' style='display:none' class='nav'><form id='svgChartPseudoForm' target='_blank' method='POST'>" +
	...
	show_editor: function (event) {
		...
		if ($currentTarget.attr("disabled") !== "disabled") {
			new ChartEditor({ data: this.renderer, workspace: this.workspace })
				.render()
				.open();
		}
	},
	...
	show: function (event, ui) {
		...
		this.renderer.cccOptions.width =
			$(this.workspace.el).find(".workspace_results").width() - 40;
		this.renderer.cccOptions.height =
			$(this.workspace.el).find(".workspace_results").height() - 40;
		...
		if ($(this.workspace.el).find("#charteditor").length === 0) {
			$(this.workspace.el)
				.find(".query_toolbar_vertical")
				.find(".options.chart.hide li:eq(0)")
				.after(
					'<li id="charteditor" class="seperator_vertical chart_editor"><a href="#charteditor" ' +
		...
		if (hasRun) {
			_.defer(function () {
				var p = self.workspace.query.getProperty(
					"saiku.ui.chart.options"
				);

				self.renderer.process_data_tree(
					{ data: self.workspace.query.result.lastresult() },
					true,
					true
				);
				self.renderer.switch_chart(self.renderer.type, {
					workspace: this.workspace,
					chartDefinition: p ? p.chartDefinition : null,
				});
			});
		}
	...
	export_button: function (event) {
		var self = this;
		var $target = $(event.target).hasClass("button")
			? $(event.target)
			: $(event.target).parent();
...
Kusai.loadCSS("js/saiku/plugins/CCC_Chart/chart_editor.css");
