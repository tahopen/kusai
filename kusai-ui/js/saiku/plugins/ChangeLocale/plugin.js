...
var ChangeLocale = Backbone.View.extend({
	initialize: function (args) {
		...
		_.bindAll(
			...
			"show",
			"handleClick",
			"processDatasourcesWithoutAddingNewTab",
			"toggleLocaleScreen",
			"setUserFeedback"
		);
		...
		this.localeOptionsScreen = $(
			this.localeOptionsScreenTemplate({
				localeMap: Settings.MONDRIAN_LOCALES,
			})
		);
	...
	add_button: function () {
		var $chart_button = $(
			...
			"background-image":
				"url('js/saiku/plugins/ChangeLocale/images/change_locale.png')",
	...
	handleClick: function (event) {
		...
		var selectedConnectionName = selectedCube.substring(
			0,
			selectedCube.indexOf("/")
		);
	...
	refreshDatasources: function () {
		...
		Kusai.session.sessionworkspace.clear();

		if (typeof localStorage !== "undefined" && localStorage) {
			localStorage.setItem("kusai-version", Settings.VERSION);
		}

		Kusai.session.sessionworkspace.fetch(
			{ success: this_plugin.processDatasourcesWithoutAddingNewTab },
			{}
		);
	},
	...
	processDatasourcesWithoutAddingNewTab: function (model, response) {
		...
		Kusai.session.sessionworkspace.cube_navigation = _.template(
			$("#template-cubes").html()
		)({
		...
		Kusai.session.sessionworkspace.connections = response;
		_.delay(Kusai.session.sessionworkspace.prefetch_dimensions, 20);
...
Kusai.events.bind("session:new", function (session) {
	function new_workspace(args) {
		if (typeof args.workspace.changeLocale == "undefined") {
			args.workspace.changeLocale = new ChangeLocale({
	...
	function clear_workspace(args) {
		if (typeof args.workspace.changeLocale != "undefined") {
			...
			$(args.workspace.changeLocale.el)
				.parents()
	...
	if (Settings.PLUGINS.indexOf("ChangeLocale") != -1) {
		...
		for (var i = 0; i < Kusai.tabs._tabs.length; i++) {
