...
var Filters = Backbone.View.extend({
	...
	render_menu: function () {
		this.repo = new FilterRepository({
			...
			data: { query: this.workspace.query.id },
		});
	},
...
*/
	...
	add_filter: function (event) {
		...
		if (queryFilter != null) {
			var selectedName =
				this.currentFilter != null ? this.currentFilter.name : null;
			...
			if (filterAction == "add") {
				$(
					...
					$(self.el).find(".filter_items .add_filter").parent()
				);
			} else {
				var edit =
					"<li><select id='new_filter' class='edit_filter_input'>";
				_.each(self.filters, function (filter) {
					edit +=
						"<option value='" +
						safe_tags_replace(filter.name) +
						"'>" +
						safe_tags_replace(filter.name) +
						"</option>";
				});
				...
				$(
					...
					$(self.el).find(".filter_items .edit_filter").parent()
				);
	...
	click_filter: function (event) {
		...
		this.currentFilter = this.filters.hasOwnProperty(filterName)
...
Kusai.events.bind("session:new", function (session) {
	function new_workspace(args) {
		...
		if (typeof args.workspace.filters == "undefined") {
			...
			args.workspace.bind(
				"query:result",
				args.workspace.filters.deactivate_add_filter
			);
		}
	}
...
var KusaiFilter = Backbone.Model.extend({
	filtername: null,
	...
	url: function () {
		return encodeURI(
			Kusai.session.username + "/filters/" + this.get("filtername")
		);
	},
});
...
var FilterRepository = Backbone.Collection.extend({
	...
	url: function () {
		return encodeURI(Kusai.session.username + "/filters/");
	},
});
