...
var Table = Backbone.View.extend({
	...
	initialize: function (args) {
		...
		this.renderer = new KusaiTableRenderer();
	...
	collapseAndHide: function ($targetRow, $rowIterator, row, col) {
		...
		while (nextRowSame) {
			if ($rowIterator.length > 0) {
				...
				if (
					isLast.length > 0 &&
					$(isLast[col]).hasClass("row_total_corner")
				) {
					nextRowSame = false;
					...
					for (var i = 0; i <= col; i++) {
						...
						if (
							row <
								self.workspace.query.result.lastresult().cellset
									.length &&
							self.workspace.query.result.lastresult().cellset[
								row
							][i].value !=
								self.workspace.query.result.lastresult()
									.cellset[row - 1][i].value &&
							!thisIsTotal
						) {
							var $lastTotalRow = null;
							while (
								$rowIterator.find(".row_total_corner").length !=
								0
							) {
								$rowIterator.hide();
							...
							if ($lastTotalRow != null) {
								var targetRowDataValues =
									$targetRow.find("div.datadiv");
								var summaryDataValues =
									$lastTotalRow.find("td.data");
								for (
									var i = 0;
									i < targetRowDataValues.length;
									i++
								) {
									var div =
										'<span class="totalSpan">' +
	...
	clicked_cell: function (event) {
		...
		//return false;
		if (
			/*this.workspace.query.get('type') != 'QM' ||*/ Settings.MODE ==
			"table"
		) {
			//return false;
		}
		if (
			$(this.workspace.el).find(".workspace_results.ui-selectable")
				.length > 0
		) {
			$(this.workspace.el)
		...
		$.contextMenu({
			...
			build: function ($trigger, e) {
				...
				var axis = $(e.currentTarget).hasClass("row")
					? "ROWS"
					: "COLUMNS";
				...
				var col = parseInt(pos[1]);
				var cell =
					self.workspace.query.result.lastresult().cellset[row][col];
				...
				if (!cubeModel || !dimensions || !measures) {
					...
					dimensions =
						Kusai.session.sessionworkspace.cube[key].get(
							"data"
						).dimensions;
				}
				var used_levels = [];
						...
						_.each(dimension.hierarchies, function (hierarchy) {
							if (hierarchy.uniqueName == h) {
								_.each(hierarchy.levels, function (level) {
									...
									items["keep-" + level.name] =
										items[level.name];
									items["include-" + level.name] = JSON.parse(
										JSON.stringify(items[level.name])
									);
									items["keep-" + level.name].payload =
										keep_payload +
										"," +
										items[level.name].payload;
								});
							}
						});
					}
				...
				if (
					items.hasOwnProperty("remove-" + l_name) &&
					items.hasOwnProperty("include-" + l_name)
				) {
					items.showall = {
						payload:
							items["remove-" + l_name].payload +
							", " +
							items["include-" + l_name].payload,
					};
				}
				...
				return {
					callback: function (key, options) {
						...
						if (key === "keeponly") {
							...
							var hierarchy =
								self.workspace.query.helper.getHierarchy(h);
							if (
								(hierarchy &&
									hierarchy.levels.hasOwnProperty(l_name)) ||
								h == "[Measures]"
							) {
								if (h == "[Measures]") {
									var measure = {
									...
									self.workspace.query.helper.includeMeasure(
										measure
									);
									...
									hierarchy.levels[l_name].selection = {
										type: "INCLUSION",
										members: updates,
									};
							...
							var lname = cell.properties.level.substring(
								cell.properties.level.lastIndexOf(".") + 1
							);
							...
							key.substring(0, key.indexOf("-")) === "remove"
						) {
							...
							key.substring(0, key.indexOf("-")) === "keep"
						) {
							//Keep and Include
							var k = key.substring(key.indexOf("-") + 1);
							...
							if (
								hierarchy &&
								hierarchy.levels.hasOwnProperty(l_caption)
							) {
								updates.push({
									uniqueName: cell.properties.uniquename,
									caption: cell.properties.uniquename,
								});
								hierarchy.levels[l_caption].selection = {
									type: "INCLUSION",
									members: updates,
								};
								self.workspace.query.helper.includeLevel(
									axis,
									h,
									k,
									null
							...
							key.substring(0, key.indexOf("-")) === "include"
						) {
							//Include
							var k = key.substring(key.indexOf("-") + 1);
							self.workspace.query.helper.includeLevel(
	...
	render: function (args, block) {
		...
		if (
			args.data === null ||
			(args.data.height && args.data.height === 0)
		) {
			return;
		}
		this.clearOut();
	...
	process_data: function (data) {
		var hideEmptyRows =
			Settings.HIDE_EMPTY_ROWS &&
			this.workspace.query.getProperty("saiku.olap.query.nonempty");
	...
	post_process: function () {
		if (
			this.workspace.query.get("type") == "QM" &&
			Settings.MODE != "view"
		) {
        ...
        */
		$(this.el).find(".i18n").i18n(Kusai.i18n.po_file);
