...
var ParentMemberSelectorModal = Modal.extend({
	...
	initialize: function (args) {
		...
		var dimensions =
			Kusai.session.sessionworkspace.cube[this.cube].get(
				"data"
			).dimensions;
		...
		this.bind("open", function () {
			...
			if (
				...
				_.isEmpty(this.breadcrumbs) &&
				!_.isEmpty(this.dimension)
			) {
	...
	populate_breadcrumbs: function (data) {
		...
		for (var i = 0; i < len; i++) {
			if (i !== data.length - 1) {
				if (i === 0 || i === 1) {
					$crumbs.push(
						'<a href="#fetch_crumb" class="crumb" data-position="' +
							i +
							'" data-action="false">' +
							data[i] +
							"</a> &gt;"
	...
	populate_members_list: function (data) {
		...
		for (var i = 0; i < len; i++) {
			var levelunique = "";
			var levelChildMember = new LevelChildMember(
				{},
				{
					ui: this,
					cube: this.cube,
					uniqueName: data[i].uniqueName,
					levelUniqueName: data[i].levelunique,
					mname: data[i].name,
					mcaption: data[i].caption,
				}
			);
			$members = $("<li />")
				.addClass("member")
				...
				.data(
					"levelUniqueName",
					data[i].levelUniqueName ? data[i].levelUniqueName : false
				)
				.data("currentLevelUnique", data[i].levelUniqueName)
				.html(
					data[i].name +
						"<span class='drill_member' style='float:right;'>Next Level</span>"
				);
	...
	get_child_members: function (model, response) {
		...
		if (response && response.length > 0) {
			...
			model.ui.breadcrumbs = _.initial(
				model.ui.breadcrumbs,
				model.ui.breadcrumbs.length - (position + 1)
			);
	...
	return_actual_level: function (model, response) {
		if (response && response.length > 0) {
			...
			model.ui.current_level = _.last(
				model.ui.actualLevel.split("].[")
			).replace(/[\[\]]/gi, "");
	...
	drill_member: function (event) {
		...
		var levelChildMember = new LevelChildMember(
			{},
			{ ui: this, cube: this.cube, uniqueName: uniqueName }
		);
	...
	get_actual_level: function (event) {
		...
		var levelChildMember = new LevelChildMember(
			{},
			{ ui: this, cube: this.cube, uniqueName: event }
		);
		levelChildMember.fetch({
	...
	fetch_crumb: function (event) {
		...
		if ($currentTarget.data("action")) {
			...
			this.reset_form();
			this.breadcrumbs = _.initial(
				this.breadcrumbs,
				this.breadcrumbs.length -
					(Number($currentTarget.data("position")) + 1)
			);
			// this.selected_level();
	...
	fetch_dimension: function (event) {
		...
		var dimension = {
			...
			dataDimension: this.$el
				.find("#dimension option:selected")
				.data("dimension"),
		};
	...
	reset_form: function () {
		...
		this.$el
			.find(".members-list")
			.append('<li class="i18n">Loading...</li>');
		this.$el.find("#auto-filter").val("");
	},
	...
	clear: function (event) {
		...
		this.breadcrumbs = _.initial(
			this.breadcrumbs,
			this.breadcrumbs.length - (position + 1)
		);
		this.populate_breadcrumbs(this.breadcrumbs);
	},
	...
	save: function (event) {
		...
		if (
			typeof this.uniqueName === "undefined" ||
			_.isEmpty(this.uniqueName)
		) {
			alertMsg +=
				"You have to choose a member for the calculated member!";
		}
		if (alertMsg !== "") {
			alert(alertMsg);
		} else {
			if (this.select_type === "parent-member-selector") {
				...
				var uniqueName =
					this.uniqueName.split(dimHier)[1] !== undefined
				...
				if (Settings.PARENT_MEMBER_DIMENSION) {
					...
					Kusai.session.trigger("ParentMemberSelectorModal:save", {
						dialog: this.dialog,
						selectedDimension: this.$el
							.find("#dimension option:selected")
							.val(),
					});
				}
