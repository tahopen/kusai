...
var CalculatedMemberModal = Modal.extend({
	...
	initialize: function (args) {
		...
		var measures =
			Kusai.session.sessionworkspace.cube[cube].get("data").measures;
		var dimensions =
			Kusai.session.sessionworkspace.cube[cube].get("data").dimensions;
		var calculatedMeasures =
			this.workspace.query.helper.getCalculatedMeasures();
		var calculatedMembers =
			this.workspace.query.helper.getCalculatedMembers();
		var $tplCalculatedMeasures = this.template_cms(
			calculatedMeasures,
			"calcmeasure"
		);
		var $tplCalculatedMembers = this.template_cms(
			calculatedMembers,
			"calcmember"
		);
		var dataMeasures = {
			name: measures
				? measures[0].dimensionUniqueName.replace(/[\[\]]/gi, "")
				: null,
			uniqueName: measures ? measures[0].hierarchyUniqueName : null,
		};

		Kusai.ui.block('<span class="i18n">Loading...</span>');
		...
		Kusai.session.bind(
			"ParentMemberSelectorModal:save",
	...
	post_render: function () {
		var tPerc =
			((($("body").height() - 570) / 2) * 100) / $("body").height();
		...
		if (data && data.length !== 0) {
			if (type === "calcmeasure") {
				_.each(data, function (value) {
					$tpl +=
						'<tr class="row-cms-' +
						self.replace_cms(value.name) +
						'">' +
						'<td class="cms-name">' +
						value.name +
						"</td>" +
						'<td class="cms-actions">' +
						'<a class="edit button sprite btn-action-edit" href="#edit_cms" data-name="' +
						value.name +
						'" data-type="calcmeasure"></a>' +
						'<a class="delete button sprite btn-action-del" href="#show_del_cms" data-name="' +
						value.name +
						'" data-type="calcmeasure"></a>' +
						"</td>" +
						"</tr>";
			...
			if (type === "calcmeasure") {
				$tpl =
					'<p class="msg-no-cms i18n">No calculated measures created</p>';
			} else {
				$tpl =
					'<p class="msg-no-cms i18n">No calculated members created</p>';
			}
		}
	...
	edit_cms: function (event) {
		...
		var cms =
			$currentTarget.data("type") === "calcmeasure"
				? this.workspace.query.helper.getCalculatedMeasures()
				: this.workspace.query.helper.getCalculatedMembers();
		...
		_.each(cms, function (value) {
			if (value.name === $currentTarget.data("name")) {
				...
				if (
					0 !==
						$(
							'#cms-format option[value="' +
								value.properties.FORMAT_STRING +
								'"]'
						).length ||
					(value.properties.FORMAT_STRING === undefined &&
						!(
							0 !==
							$(
								'#cms-format option[value="' +
	...
	show_del_cms: function (event) {
		...
		var cmsType =
			$currentTarget.data("type") === "calcmeasure"
				? '<span class="i18n">measure</span>'
				: '<span class="i18n">member</span>';
		this.$delcms = $currentTarget;
		this.new();
		new WarningModal({
			title: '<span class="i18n">Delete Member</span>',
			message:
				'<span class="i18n">You want to delete this</span> ' +
				cmsType +
				" <b>" +
	...
	del_cms: function (args) {
		args.$delcms
			.parent()
			.closest(".row-cms-" + args.replace_cms(args.$delcms.data("name")))
			.remove();

		if (args.$delcms.data("type") === "calcmeasure") {
			args.workspace.query.helper.removeCalculatedMeasure(
				args.$delcms.data("name")
			);
		} else {
			args.workspace.query.helper.removeCalculatedMember(
				args.$delcms.data("name")
			);
		}
	...
	trigger_input_name: function () {
		...
		var dimensionDataType = this.$el
			.find("#cms-dimension option:selected")
			.data("type");
		var alertMsg = "";

		if (dimensionDataType === "calcmeasure") {
			if (
				this.check_name_cms(dimensionDataType, name) &&
				formAction === "cad"
			) {
			...
			if (
				...
				alertMsg =
					"Exists a measure or member with the same name added!";
				// this.$el.find('#cms-name').focus();
			}
		}
	...
	openGrowthModal: function (event) {
		var selectedHierarchies = this.workspace.query.helper
			.model()
			.queryModel.axes.ROWS.hierarchies.concat(
				this.workspace.query.helper.model().queryModel.axes.COLUMNS
					.hierarchies
			);
	...
	openFormatModal: function (event) {
		var selectedMeasures =
			this.workspace.query.helper.model().queryModel.details.measures;
		this.close();
	...
	save: function (event) {
		...
		if (typeof formula === "undefined" || formula === "" || !formula) {
			alertMsg +=
				"You have to enter a MDX formula for the calculated member! ";
		}
		if (
			typeof dimension.val === "undefined" ||
			dimension.val === "" ||
			!dimension.val
		) {
			alertMsg +=
				"You have to choose a dimension for the calculated member! ";
		}
		if (alertMsg !== "") {
			alert(alertMsg);
		} else {
			if (dimension.dataType === "calcmeasure") {
				...
				if (formAction === "cad") {
					...
					this.workspace.query.helper.editCalculatedMeasure(
						nameOld,
						objMember
					);
					this.workspace.sync_query();
				...
				if (this.pmUniqueName && !_.isEmpty(this.pmUniqueName)) {
					...
					objMember.uniqueName =
						this.pmUniqueName + ".[" + name + "]";
					objMember.assignedLevel = this.actualLevel;
				}
				this.workspace.query.helper.includeCmember(
					dimension.val,
					objMember.uniqueName
				);
				if (formAction === "cad") {
					...
					this.workspace.query.helper.removeLevelCalculatedMember(
						dimension.val,
						"[" + dimension.txt + "].[" + nameOld + "]"
					);
					this.workspace.query.helper.editCalculatedMember(
						nameOld,
						objMember
					);
					this.workspace.sync_query();
		...
		var functions = [
			{
				...
				description:
					"Insert a formula with an ISEMPTY check to ensure that only non null cells are" +
					" calculated",
				doc_link:
					"http://kusai-documentation.readthedocs.io/en/latest/calculated_members.html?highlight=calculated%20members",
			},
			{
				name: "Aggregate",
			...
			},
			{
				...
				doc_link:
					"https://msdn.microsoft.com/en-us/library/ms145584.aspx",
			},
			{
				name: "Cousin",
				example:
				...
				doc_link:
					"https://msdn.microsoft.com/en-us/library/ms145481.aspx",
			},
			{
				name: "CurrentMember",
				...
				doc_link:
					"https://msdn.microsoft.com/en-us/library/ms144948.aspx",
			},
			{
				name: "FirstChild",
				...
				doc_link:
					"https://msdn.microsoft.com/en-us/library/ms144947.aspx",
			},
			{
				name: "FirstSibling",
				...
				doc_link:
					"https://msdn.microsoft.com/en-us/library/ms145956.aspx",
			},
			{
				name: "IIf",
				...
				doc_link:
					"https://msdn.microsoft.com/en-us/library/ms145994.aspx",
			},
			{
				name: "LastChild",
				...
				doc_link:
					"https://msdn.microsoft.com/en-us/library/ms145576.aspx",
			},
			{
				name: "LastSibling",
				...
				doc_link:
					"https://msdn.microsoft.com/en-us/library/ms144863.aspx",
			},
			{
				name: "Max",
				...
				doc_link:
					"https://msdn.microsoft.com/en-us/library/ms145601.aspx",
			},
			{
				name: "Median",
				...
				doc_link:
					"https://msdn.microsoft.com/en-us/library/ms145570.aspx",
			},
			{
				name: "Min",
				...
				doc_link:
					"https://msdn.microsoft.com/en-us/library/ms145600.aspx",
			},
			{
				name: "MTD",
				...
				doc_link:
					"https://msdn.microsoft.com/en-us/library/ms144753.aspx",
			},
			{
				name: "OpeningPeriod",
				example:
				...
				doc_link:
					"https://msdn.microsoft.com/en-us/library/ms145992.aspx",
			},
			{
				name: "ParallelPeriod",
				example:
			...
			},
			{
				...
				doc_link:
					"https://msdn.microsoft.com/en-us/library/ms144719.aspx",
			},
			{
				name: "QTD",
				...
				doc_link:
					"https://msdn.microsoft.com/en-us/library/ms145978.aspx",
			},
			{
				name: "Sum",
				...
				doc_link:
					"https://msdn.microsoft.com/en-us/library/ms145484.aspx",
			},
			{
				name: "WTD",
				...
				doc_link:
					"https://msdn.microsoft.com/en-us/library/ms144930.aspx",
			},
			{
				name: "YTD",
				...
				description:
					"Returns a set of sibling members from the same level as a given member, starting with the first sibling and ending with the given member, as constrained by the Year level in the Time dimension.",
				doc_link:
					"https://msdn.microsoft.com/en-us/library/ms146039.aspx",
			},
		];
	...
	change_function_list: function (event) {
		...
		var selectedFunction = this.$el.find(
			".cms-functionlist option:selected"
		);
		...
		if ($(selectedFunction).data("doc-link")) {
			$(".cms-doclink").prop(
				"href",
				$(selectedFunction).data("doc-link")
			);
			$(".cms-doclink").show();
		} else {
			$(".cms-doclink").hide();
