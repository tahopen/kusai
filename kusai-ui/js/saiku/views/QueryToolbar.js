...
var QueryToolbar = Backbone.View.extend({
	...
	switch_render_button: function (event) {
		...
		var isMap =
			$(this.el).find("ul.chart li a.on:first").size() > 0
				? $(this.el)
						.find("ul.chart li a.on:first")
						.attr("href")
						.replace("#", "")
				: null;

		if ($target.hasClass("render_chart")) {
			Kusai.events.trigger("queryToolbar:render_chart", this, null);
			if (isMap === "map") {
				var mapProperties = this.workspace.query.getProperty(
					"saiku.ui.map.options"
				);
				var mapType = mapProperties
					? mapProperties.mapDefinition.type
					: "";
				this.switch_render("map");
				this.workspace.query.setProperty("saiku.ui.render.mode", "map");
				this.workspace.query.setProperty(
					"saiku.ui.render.type",
					mapType
				);
			} else {
	...
	switch_render: function (render_type) {
		render_type =
			typeof render_type != "undefined"
				? render_type.toLowerCase()
				: "table";
		$(this.el).find("ul.renderer a.on").removeClass("on");
		$(this.el)
			.find("ul.renderer a.render_" + render_type)
			.addClass("on");
		if ("chart" == render_type) {
			...
			$(this.workspace.el).find(".workspace_results").children().hide();
			$(this.workspace.el)
				.find(".workspace_results .table_wrapper")
				.show();
	...
	call: function (event) {
		...
		if (!$target.hasClass("disabled_toolbar")) {
			...
			if (this.render_mode == "table" && this[callback]) {
				...
				this.workspace.chart.$el
					.find(".canvas_wrapper")
					.find(".map-render")
					.data("action", "querytoolbar");
				if ($target.hasClass("chartoption")) {
					...
					this.workspace.querytoolbar.$el
						.find('ul.chart [href="#export_button"]')
						.parent()
						.removeAttr("disabled");
					this.workspace.querytoolbar.$el
						.find("ul.chart > li#charteditor")
						.removeAttr("disabled");
					this.workspace.querytoolbar.$el
						.find('ul.chart [href="#map"]')
						.removeClass("on");
					$target
						.parent()
						.siblings()
	...
	render_row_viz: function (type) {
		$(this.workspace.table.el)
			...
			.each(function (index, element) {
				...
				$(
					"<td class='data spark'>&nbsp;<div id='chart" +
						index +
						"'></div></td>"
				).appendTo($(element));
				...
				if (rowData.length > 0) {
					...
					if (type == "spark_bar") {
						vis.add(pv.Bar)
							.data(rowData)
							.left(
								pv.Scale.linear(0, rowData.length)
									.range(0, width)
									.by(pv.index)
							)
							.height(
								pv.Scale.linear(0, _.max(rowData)).range(0, 12)
							)
							.width(6)
							.bottom(0);
						...
						vis.add(pv.Line)
							.data(rowData)
							.left(
								pv.Scale.linear(0, rowData.length - 1)
									.range(0, width)
									.by(pv.index)
							)
