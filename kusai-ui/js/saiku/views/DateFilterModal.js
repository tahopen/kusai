...
var DateFilterModal = Modal.extend({
	...
	template_days_mdx:
		"Filter({parent}.Members, {parent}.CurrentMember.NAME {comparisonOperator} '{dates}'",

	template_many_years_mdx:
		" {logicalOperator} {parent}.CurrentMember.NAME {comparisonOperator} '{dates}'",

	template_mdx:
		"IIF(ISEMPTY(CurrentDateMember([{dimension}.{hierarchy}]," +
	...
	template_last_mdx:
		"{parent} LastPeriods({periodAmount}, CurrentDateMember([{dimension}.{hierarchy}], '[\"{dimension}.{hierarchy}\"]\\.{analyzerDateFormat}', EXACT))",
	...
	initialize: function (args) {
		...
		// Check KusaiDayFormatString in level
	...
	help: function (event) {
		...
		window.open(
			"http://kusai-documentation.readthedocs.io/en/latest/advanced_date_filtering.html"
		);
	},
	...
	show_fields: function (event) {
		...
		if (name !== undefined) {
			switch (name) {
				...
				case "different":
					$currentTarget
						.closest(".selection-options")
						.find("#div-selection-date")
						.show();
					$currentTarget
						.closest(".selection-options")
						.find("#div-selected-date")
						.show();
					$currentTarget
						.closest(".selection-options")
						.find("#div-select-start-date")
						.hide();
					$currentTarget
						.closest(".selection-options")
						.find("#div-select-end-date")
						.hide();
					$currentTarget
						.closest(".selection-options")
						.find("#add-date")
	...
	save_data_levels: function () {
		...
		_.each(this.data.hierarchies.levels, function (value, key, list) {
			if (
				list[key].annotations.AnalyzerDateFormat !== undefined ||
				list[key].annotations.KusaiDayFormatString !== undefined
			) {
				if (list[key].annotations.AnalyzerDateFormat !== undefined) {
					dataLevels.push({
						name: list[key].name,
						analyzerDateFormat: list[
							key
						].annotations.AnalyzerDateFormat.replace(
							/[.]/gi,
							"\\."
						),
						levelType: list[key].levelType,
						saikuDayFormatString:
							list[key].annotations.KusaiDayFormatString || "",
					});
				} else {
					dataLevels.push({
						name: list[key].name,
				...
				if (list[key].annotations.KusaiDayFormatString) {
					self.saikuDayFormatString =
						list[key].annotations.KusaiDayFormatString;
				}
			}
		});
	...
	disable_divselections: function (event) {
		...
		this.$el
			...
			.off("click");
		$currentTarget
			.closest(".box-selections")
			.find(".available-selections")
			.attr("available", true);
		$currentTarget
			.closest(".box-selections")
			.find(".available-selections *:not(.keep-disabled)")
			.prop("disabled", false)
			.on("click");
	...
	add_selected_date: function (event) {
		...
		var $currentTarget = $(event.currentTarget),
			...
			selectedDate = $currentTarget
				.closest(".inline-form-group")
				.find("#div-selected-date")
				.find("#selected-date");

		if (sDate.val() !== "") {
			var newDate = Kusai.toPattern(sDate.val(), dayFormatString);
			sDate.css("border", "1px solid #ccc");
			selectedDate.append(
				$("<li></li>")
					.text(newDate)
					.append(
						'<a href="#" class="del-date" data-date="' +
							newDate +
							'">x</a>'
					)
			);
			...
			if (data.type === "operator") {
				...
				if (
					...
					_.each(this.selectedDates, function (value, key) {
						self.$el.find("#selected-date").append(
							$("<li></li>")
								.text(value)
								.append(
									'<a href="#" class="del-date" data-date="' +
				...
				this.$el
					.find(
						'select#period-select option[id="' +
							data.periodSelect +
							'"]'
					)
					.prop("selected", true);
	...
	populate_mdx: function (logExp, fixedDateName, periodAmount) {
		logExp.tagdim = logExp.dimension
			.replace(/m/g, "\\m")
			.replace(/y/g, "\\y")
			.replace(/q/g, "\\q")
			.replace(/d/g, "\\d");
		logExp.taghier = logExp.hierarchy
			.replace(/m/g, "\\m")
			.replace(/y/g, "\\y")
			.replace(/q/g, "\\q")
			.replace(/d/g, "\\d");
		...
		if (
			logExp.workinglevel !== logExp.level &&
			logExp.workinglevel !== undefined
		) {
		...
		if (fixedDateName === "dayperiods") {
			...
			if (this.selectedDates.length > 1) {
					...
					if (logExp.comparisonOperator === ">=|<=") {
						if (i === 0) {
							logExp.comparisonOperator =
								logExp.comparisonOperator.split("|")[0];
							this.template_days_mdx =
								this.template_days_mdx.replace(
									/{(\w+)}/g,
									function (m, p) {
										return logExp[p];
									}
							...
							logExp.comparisonOperator =
								logExp.comparisonOperator.split("|")[1];
							this.template_days_mdx +=
								this.template_many_years_mdx.replace(
									/{(\w+)}/g,
									function (m, p) {
										return logExp[p];
									}
				...
				this.template_days_mdx =
					this.template_days_mdx.replace(/{(\w+)}/g, function (m, p) {
						return logExp[p];
					}) + ")";
			...
			this.template_last_mdx = this.template_last_mdx.replace(
				/{(\w+)}/g,
				function (m, p) {
					return logExp[p];
				}
			...
			this.template_mdx = this.template_mdx.replace(
				/{(\w+)}/g,
				function (m, p) {
					return logExp[p];
				}
	...
	save: function (event) {
		...
		this.$el.find(".available-selections").each(function (key, selection) {
			if ($(selection).attr("available") === "true") {
				...
				if ($(selection).attr("selection-name") === "operator") {
						...
						.each(function (key, radio) {
							if ($(radio).is(":checked") === true) {
								...
								if (
									...
									self.selectedDates = [];
									self.selectedDates.push(
										self.$el.find("#selection-date").val()
									);
								} else if (
									name === "between" ||
									name === "notbetween"
									...
									self.selectedDates = [];
									self.selectedDates.push(
										self.$el.find("#start-date").val()
									);
									self.selectedDates.push(
										self.$el.find("#end-date").val()
									);
								}
					...
					$(selection).attr("selection-name") === "fixed-date"
				) {
					$(selection)
						.find("input:radio")
						.each(function (key, radio) {
					...
					$(selection).attr("selection-name") === "rolling-date"
				) {
					analyzerDateFormat = $("#period-select")
						.find(":selected")
						.val();
					fixedDateName = "lastperiods";
					periodAmount = $(selection).find("input:text").val();
					selectedData.fixedDateName = fixedDateName;
					selectedData.periodAmount = $(selection)
						.find("input:text")
						.val();
					selectedData.periodSelect = $("#period-select")
						.find(":selected")
						.attr("id");
				}
				...
				for (i = 0; i < len; i++) {
					if (
						self.dataLevels[i].analyzerDateFormat ===
						analyzerDateFormat
					) {
				...
				if (
					...
					(fixedDateName === "lastperiods" &&
						!_.isEmpty(analyzerDateFormat) &&
						analyzerDateFormat !== "Day(s)" &&
						!_.isEmpty(periodAmount)) ||
					(fixedDateName !== "dayperiods" &&
						fixedDateName !== "lastperiods" &&
						!_.isEmpty(analyzerDateFormat))
				) {
		...
		if (
			...
			this.workspace.query.setProperty(
				"saiku.ui.datefilter.data",
...
var DateFilterObserver = Backbone.View.extend({
	initialize: function (args) {
		...
		Kusai.session.bind("dimensionList:select_dimension", this.receive_data);
		Kusai.session.bind(
			"workspaceDropZone:select_dimension",
			this.receive_data
		);
		Kusai.session.bind("workspaceDropZone:clear_axis", this.receive_data);
	},
	...
	workspace_levels: function (args) {
		...
		if (
			axisColumns.location === "COLUMNS" &&
			axisColumns.hierarchies.length > 0
		) {
		...
		if (
			axisFilter.location === "FILTER" &&
			axisFilter.hierarchies.length > 0
		) {
	...
	check_dateFilter_saved: function () {
		...
		if (
			this.workspace.item &&
			!_.isEmpty(this.workspace.item) &&
			checkDateFilterSaved === undefined
		) {
			var data = this.workspace.query.getProperty(
				"saiku.ui.datefilter.data"
			);
...
Kusai.events.bind("session:new", function () {
	function new_workspace(args) {
		if (typeof args.workspace.dateFilterObserver === "undefined") {
			args.workspace.dateFilterObserver = new DateFilterObserver({
