		...
		var st = [
			{
				...
				setup: function (tour, options) {
					//options.view.reset();
					Kusai.events.bind("toolbar:new_query", this.onSelectKitten);
					//options.view.enable();
				},
			...
			},
			{
				...
				setup: function (tour, options) {
					...
					Kusai.events.bind("workspace:new_query", this.onSelectCube);
					//options.view.enable();
				},
				teardown: function (tour, options) {
					// Disallow more kitten selection
				...
				onSelectCube: function (tour, options, view, kitten) {
					if (kitten.cube === "Sales") {
						...
						$(".tour-container").html(
							"<p>You didn't select the Sales cube.</p>"
						);
					}
				},
			},
			...
			},
			{
				...
				setup: function (tour, options) {
					Kusai.events.bind(
						"workspace:expandDimension",
						this.onExpandDimension
					);
			...
			{
				content:
					"<p>Great next drag the State Province level to the Rows dropzone.</p>",
				highlightTarget: true,
				nextButton: false,
				closeButton: true,
				target: $(".rows_fields"),
				...
				onSelectLevel: function (tour, options, view, kitten) {
					if (
						kitten.hierarchy === "[Customer].[Customers]" &&
						kitten.level === "State Province"
					) {
						tour.next();
					} else {
						$(".tour-container").html(
							"<p>You didn't select the correct level. " +
			...
			{
				content:
					"<p>Next we'll add a measure. Drag Unit Sales into the Measures Drop Zone.</p>",
				highlightTarget: true,
				nextButton: false,
				closeButton: true,
				target: $(".rows_fields"),
				...
				setup: function (tour, options) {
					Kusai.events.bind(
						"workspaceDropZone:select_measure",
						this.onSelectMeasure
					);
				...
				onSelectMeasure: function (tour, options, view, kitten) {
					if (kitten.measure.name === "Unit Sales") {
						...
						$(".tour-container").html(
							"<p>You didn't select the correct measure. " +
			...
			{
				content:
					"<p>Great, we've now run a very basic query. Now we'll view it as a chart.</p>",
				highlightTarget: true,
				nextButton: true,
				closeButton: true,
				target: $(".rows_fields"),
			...
			{
				content:
					"<p>To view your data as a chart, click on the chart icon.</p>",
				highlightTarget: true,
				nextButton: false,
				closeButton: true,
				target: $(".rows_fields"),
			...
			{
				content:
					"<p>You can now select from the different chart type Kusai offers.</" +
					"p>Kusai Enterprise users can set chart properties by clicking on the properties button.</p>" +
			...
			{
				content:
					"<p>On the workspace toolbar there are a number of useful buttons.</p>",
				highlightTarget: true,
				nextButton: true,
				closeButton: true,
				target: $(".rows_fields"),
			...
			{
				content:
					"<p>Edit Query. This button will hide the query design area on the left of the workspace.</p>",
				highlightTarget: true,
				nextButton: true,
				closeButton: true,
				target: $(".rows_fields"),
			...
			{
				content:
					"<p>Run Query/Automatic Execution. These icons allow you to manually run a query, or," +
			...
			{
				content:
					"<p>Hide Parents. This button allows you to show the parent levels in the result set.</p>" +
			...
			{
				content:
					"<p>Non Empty. This button hides null fields so minimise the resultset. Click this button" +
			...
			{
				content:
					"<p>Zoom. Zoom into a large table, by dragging over the area you want to show.</p>",
				highlightTarget: true,
				nextButton: true,
				closeButton: true,
				target: $(".rows_fields"),
			...
			{
				content:
					"<p>Drill Across. A way to discover more about your data. Select drill across and enable" +
				...
				setup: function (tour, options) {
					return {
						target: $('a[original-title="Drill across on cell"]'),
					};
				},
			},
			{
				content:
					"<p>Drill Through. Drill through to the lowest level to show how your query was constructed.</p>",
				highlightTarget: true,
				nextButton: true,
				closeButton: true,
				target: $(".rows_fields"),
				...
				setup: function (tour, options) {
					return {
						target: $('a[original-title="Drill through on cell"]'),
					};
				},
			},
			{
				content:
					"<p>We've now run through the basics that Kusai Analytics has to offer. There is far more" +
					...
					'<p>For more help, visit <a href="http://kusai-documentation.readthedocs.io/en/latest" target="_blank">http://kusai-documentation.readthedocs.io/en/latest</a>.</p>',
				highlightTarget: true,
				nextButton: true,
				closeButton: true,
				target: $(".rows_fields"),
