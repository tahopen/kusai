...
var TabSet = Backbone.View.extend({
	...
	render: function () {
		$(this.el)
			.html(
				'<a href="#pager" class="pager sprite"></a><ul><li class="newtab"><a class="new">+&nbsp;&nbsp;</a></li></ul>'
			)
			.appendTo($("#header"));
	...
	add: function (content, close) {
		...
		Kusai.session.trigger("tab:add", { tab: tab });
		this.pager.render();
		Kusai.i18n.translate();
		Kusai.session.trigger("workspace:toolbar:render", null);
		return tab;
	},
	...
	select: function (tab) {
		...
		Kusai.session.tabSelected = tab.id;
		Kusai.session.trigger("tab:select", { tab: tab });
	...
	remove: function (tab) {
		...
		for (var i = 0, len = this._tabs.length; i < len; i++) {
			if (this._tabs[i] == tab) {
				...
				Kusai.session.trigger("tab:remove", { tab: tab });
				this.pager.render();
	...
	close_others: function (tab) {
		...
		while (1 < this._tabs.length) {
			if (this._tabs[i] != tab) this._tabs[i].remove();
			else i++;
		}
	},
	...
	duplicate: function (tab) {
		...
		Kusai.ui.block("Duplicating tab...");
