...
var WorkspaceToolbar = Backbone.View.extend({
	...
	reflect_properties: function () {
		...
		if (properties["org.kusai.connection.scenario"] !== true) {
		...
		if (
			properties["saiku.olap.query.limit"] == "true" ||
			properties["saiku.olap.query.filter"] === true
		) {
		...
		if (
			$(this.workspace.el).find(".workspace_results tbody.ui-selectable")
				.length > 0
		) {
	...
	automatic_execution: function (event) {
		// Change property
		var newState = !this.workspace.query.getProperty(
	...
	non_empty: function (event) {
		// Change property
		var nonEmpty = !this.workspace.query.getProperty(
	...
	check_modes: function (source) {
		...
		if (!$(source).hasClass("on")) {
			...
			if ($(source).hasClass("drillthrough_export")) {
				...
				$(this.workspace.el)
					.find(
						".query_scenario, .drillthrough, .zoom_mode, .drillacross"
					)
					.removeClass("on");
				...
				$(this.workspace.el)
					.find(
						".query_scenario, .drillthrough_export, .zoom_mode, .drillacross"
					)
					.removeClass("on");
				...
				$(this.workspace.el)
					.find(
						".drillthrough, .drillthrough_export, .zoom_mode, .drillacross"
					)
					.removeClass("on");
				...
				$(this.workspace.el)
					.find(
						".query_scenario, .drillthrough, .drillthrough_export, .zoom_mode"
					)
					.removeClass("on");
						...
						stop: function (event, ui) {
							...
							if (positions.length > 0) {
								self.workspace.query.action.put("/zoomin", {
									...
									data: {
										selections: JSON.stringify(positions),
									},
								});
							}
						},
				...
				$(this.workspace.el)
					.find(
						".drillthrough, .drillthrough_export, .query_scenario, .drillacross, .about"
					)
					.removeClass("on");
			}
		}
	...
	export_file: function (type) {
		...
		if (this.workspace.query.name) {
			filename = this.workspace.query.name.substring(
				this.workspace.query.name.lastIndexOf("/") + 1
			);
	...
	export_csv: function (event) {
		this.export_file("csv");
	},

	export_pdf: function (event) {
		this.export_file("pdf");
	},
	...
	switch_to_mdx: function (event) {
		...
		$(this.el)
			.find(
				".auto, .query_scenario, .buckets, .non_empty, .swap_axis, .mdx, .switch_to_mdx, .zoom_mode, .drillacross"
			)
			.parent()
			.hide();
		...
		if (
			...
			$(this.workspace.el)
				.find(
					".workspace_editor .mdx_input, .workspace_editor .editor_info, .workspace_editor"
				)
				.removeClass("hide")
				.show();
			...
			var heightUpdateFunction = function () {
				...
				var height = Math.floor(
					max_height / self.editor.renderer.lineHeight
				);
				var screen_length =
					self.editor.getSession().getScreenLength() > height
						? height
						: self.editor.getSession().getScreenLength();
				var newHeight =
					(screen_length + 1) * self.editor.renderer.lineHeight +
			...
			var resizeFunction = function () {
				...
				if (session.getUseWrapMode()) {
					...
					var contentWidth =
						self.editor.renderer.scroller.clientWidth;

					if (contentWidth > 0) {
						session.setWrapLimitRange(
							null,
							parseInt(contentWidth / characterWidth, 10)
						);
					}
				}
			};
		...
		if (this.workspace.dimension_list) {
			$(this.workspace.el)
				...
				.parent("li")
				.removeClass(
					"ui-draggable ui-draggable-disabled ui-state-disabled"
				);
		}
