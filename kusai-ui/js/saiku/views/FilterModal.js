...
var FilterModal = Modal.extend({
	...
	expression_text: function () {
		var c = "<div class='sidebar'><table" + " border='0px'>";
		if (this.expressionType == "Order") {
			c +=
				"<tr><td class='col1'><label>Sort Type</label> <select class='form-control' id='fun'><option>ASC</option><option>BASC</option><option>DESC</option><option>BDESC</option> </select></td></tr>";
		}
		c +=
			"<tr><td class='col1'>" +
			this.expressionType +
			" MDX Expression:</td></tr>" +
			"<tr><td class='col1' style='width:380px'><div class='filter-editor' style='width:380px' id='" +
			this.id +
			"'></div></td></tr>" +
			"</table>" +
	...
	save: function (event) {
		...
		var alert_msg = "";
		if (
			typeof this.expression == "undefined" ||
			!this.expression ||
			this.expression === ""
		) {
			alert_msg +=
				"You have to enter a MDX expression for the " +
				this.expressionType +
				" function! ";
			alert(alert_msg);
		} else {
	...
	help: function () {
		window.open(
			"http://kusai-documentation.readthedocs.io/en/latest/filtering.html"
		);
	},
});
