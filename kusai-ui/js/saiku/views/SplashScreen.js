...
var SplashScreen = Backbone.View.extend({
	...
	template: _.template(
		'<div id="splash">' +
			...
			"<p>Kusai provides the user with an entirely browser based experience. We support all modern browsers, and our user interface is 100% HTML and Javascript.<br/><br/>Kusai uses REST based communications, this allows the development of custom user interfaces and facilitates the easy integration of the Kusai Server into other applications and services.</p>" +
			"<h3>Standards Compliant</h3>" +
			"<p>Kusai is based upon the Microsoft MDX query language and will work on most JDBC compliant data sources. We also provide a number of connectors to NOSQL data sources.</p>" +
			"<h3>Dynamic charting</h3>" +
			"<p>Kusai uses a flexible charting engine to provide a wide range of charts and graphs. These are all HTML & Javascript only and don't require flash to be installed on the computer.</p>" +
			"<h3>Pluggable visualisation engine</h3>" +
			"<p>Kusai Enterprise boasts a fully pluggable visualisation engine. This allows developers to build third party extensions and plug them into Kusai Enterprise to extend or replace the existing visualisations.</p>" +
			"</section>" +
			'<section style="display: none !important; margin-top: 50px;" id="help">' +
			'<h1 class="saikulogo">Kusai</h1>' +
			"<h2>Help</h2>" +
			"<p>We provide Training, Consulting and Support to ensure you get the most from Kusai and your data. Our services cover all aspects of data analysis including data strategy, design, architecture, deployment and application/software support.</p>" +
			...
			"</ul>" +
			"</td>" +
			'<td>Why not try  our new <a href="http://kusai-documentation.readthedocs.io/en/latest/" target="_blank">Documentation site</a><br/>for community documentation.</td>' +
	...
	run_dashboard: function () {
		if (Kusai.Dashboards === undefined) {
			alert("Please upgrade to Kusai Enterprise for Dashboards");
		} else {
	...
	hide_content_enterprise: function () {
		...
		this.$el.find("#splash #welcome").css({
			background:
				'url("../../../images/src/mac.png") top right no-repeat',
		});
	},
