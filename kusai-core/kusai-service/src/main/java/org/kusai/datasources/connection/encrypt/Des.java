package org.kusai.datasources.connection.encrypt;
...
public final class Des {
    ...
    public int SetKey(boolean encrypt,
            ...
            byte[] key2) {
        /*
         * Method to set 2 keys, thus invoking triple DES mode. Parameters: Encrypt - 0
         * to decrypt the data, else encrypt it. key1 - the
         * first key, parity bits ignored. key2 - second key, parity bits ignored.
         * Returns: 0 if either key is 'weak' or NULL, else all
    ...
    /***********************************************************************************************************************************
     * Perform an encryption/decryption operation, which being determined by how the
     * keys were specified. The length must be a multiple
     * of the block size - 64 bits or 8 bytes. Anything else is reject, so it is up
     * to the caller to manage that.
     *
     * Parameters: pvTo - destination pvFrom - source, may be the same as pvTo. cb -
     * number of bytes to transform, multiple of 8.
     ...
     * +++ CDES::Munge64()
     * 
     * Encrypt/Decrypt 64 bits of data. This is the guts of the algorithm, being the
     * method which does the actual work. This is,
     * perhaps, not as clear as might be desired, but speed is somewhat important
     * here, as there is a great deal of work involved with
     * the actual transformation.
     * 
     * Parameters: pOut - place where data is stored, byte aligned. pIn - source of
     * data, byte aligned, maybe the same as pOut. Returns:
    ...
    private void Munge64(byte[] pOut,
        ...
        /*
         * Perform a circular left shift of both halves in here. The reason is to remove
         * it from the inner loop which follows. Without
         * this step, each iteration would involve another circular shift at the
         * beginning.
         */
        ...
        /*
         * Undo the circular left shift which has been in force for this entire loop.
         * It's now time to return data, and unshifted too!
         */
        ...
        /*
         * The final bit permutation. This is a little messy, since each output byte
         * contains a bit from each of the 8 bytes we have
         * just permuted.
         */
     ...
     * +++ CDES::DoDes()
     * 
     * Private method to perform the inner most computation of the DES algorithm.
     * This performs one comlete iteration through DES, so
     * data passed in is returned in a completed state. For triple DES, this method
     * would be invoked three times per 64 bit block.
     * Requires some processing of the data before reaching here.
     * 
     * Parameters: pL - the 'left' half of the 64 bits to transform. pR - the
     * 'right' half of the 64 bits to transform. pKey - the key
    ...
    private void DoDes(int[] Halves,
        ...
        for (ii = 0; ii < DES_ROUNDS / 2; ++ii) {
            /*
             * There is no need to perform the expansion permutation here, because we have
             * arranged the data to be in that state
             * already. The expansion takes a 4 bit nibble and includes a bit from each of
             * the adjacent nibbles (with end around carry).
             * The input halves are already adjusted with the end around carry (a 1 bit
             * right circular shift), so by using 6 bits of
             * each byte, we are in fact working with the expanded data, for each byte. By
             * then circular shifting left 4 bits, and again
             * using 6 bits from each byte, we have the remaining half of the expanded data.
             * The key data has been generated to have
             * this same format, hence we can just use an exor, then use the S box
             * substitution. The S box tables are also adjusted with
     ...
     * +++ CDES::KeyPermute()
     * 
     * Applies the key permutation operations to the 56 bit input key. Also checks
     * for a weak key, where this is defined as either half
     * being all 1s or all 0s (56 bits in each case). Because these are used in EXOR
     * ops with the input, such values will result in no
     * overall change.
     * 
     * Parameters: pKey - output data. pchKeyIn - the input key, 7 bytes, parity bit
     * removed. Returns: 0 for a weak key, else the key is
    ...
    private boolean KeyPermute(int[] pKey,
        ...
        /*
         * Check for a weak key. There is an array of these. There are exactly 64 of
         * them.
         */
     ...
     * +++
     * 
     * Parameters: pRound - the array to fill with the transformed keys. pKey - .
     * Encrypt - 1 if for encryption, 0 for decryption.
     * Returns: Nothing, failure is not meaningful. ---
     */
    /**
     *
     * Generate the keys used during each round. There are several variations,
     * depending upon encryption/decryption key, and which of
     ...
     * @param encrypt
     *                <code>true</code> for encryption, <code>false</code> for
     *                decryption
     */
    private void RoundGen(int[][] pRound,
        ...
        for (ii = 0; ii < DES_ROUNDS; ++ii) {
            ...
            /*
             * Each round uses just 48 of these 56 bits, so now pick those bits actually
             * used. Note that the following loop(s) actually
             * process all 56 bits. Those bits which are not used are set in one of the
             * unused bits in the output - currently bit 6.
             * This does not affect the operation, as the inner loop uses only the low 6
             * bits of each byte when feeding data to the
             * S-Box/P-Box tables.
             */
            ...
            /*
             * NOTE: This loop is split in two, as it makes the code quite a bit faster,
             * avoiding a division and remainder operation for
             * each iteration.
             */
