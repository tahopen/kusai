...
import org.kusai.olap.dto.*;
import org.kusai.olap.dto.filter.SaikuFilter;
import org.kusai.olap.dto.resultset.AbstractBaseCell;
import org.kusai.olap.dto.resultset.CellDataSet;
import org.kusai.olap.dto.resultset.DataCell;
import org.kusai.olap.dto.resultset.MemberCell;
import org.kusai.olap.query.*;
...
import org.kusai.olap.util.formatter.CellSetFormatter;
import org.kusai.olap.util.formatter.FlattenedCellSetFormatter;
import org.kusai.olap.util.formatter.HierarchicalCellSetFormatter;
...
public class OlapQueryService implements Serializable {
	...
	private CellDataSet execute(String queryName, ICellSetFormatter formatter) {
		...
		try {
			...
			// we could do a check if query.getTotalFunctions() actually includes a total
			// function and if not dont execute the following
		...
		if (t.getSaikuDimensionSelections() != null) {
			for (SaikuDimensionSelection dimsel : t.getSaikuDimensionSelections()) {
				if (!dimsel.getName().equals("Measures")) {
					...
					// TODO: Move it to columns since drilling through with 2 filter items of the
					// same dimension doesn't work
	...
	public List<SimpleCubeElement> getResultMetadataMembers(String queryName, boolean preferResult,
		...
		if (cs == null || !preferResult || members.size() == 0 || levels.size() == 1) {
			members = olapDiscoverService.getLevelMembers(query.getSaikuCube(), hierarchyName, levelName, searchString,
					searchLimit);
		}
		return members;
	}
	...
	public ResultSet drillthrough(String queryName, String drillthroughMdx) {
		...
		try {
			...
			throw new SaikuServiceException("Error DRILLTHROUGH: " + queryName + " DRILLTHROUGH MDX:" + drillthroughMdx,
					e);
		} finally {
			try {
				if (stmt != null)
	...
	public boolean removeLevel(String queryName, String dimensionName, String uniqueHierarchyName,
		...
		try {
			...
			throw new SaikuServiceException("Cannot remove level" + uniqueLevelName + "from dimension " + dimensionName,
					e);
		}
		return true;
	}
	...
	public SaikuFilter getFilter(String queryName, String filtername, String dimensionName, String hierarchyName,
		...
		try {
			...
			if (mset.size() == 0) {
				...
				// if (hierarchy == null) {
				// hierarchy = new SimpleCubeElement(he.getName(), he.getUniqueName(),
				// he.getCaption());
				// dimension = new SimpleCubeElement(d.getName(), d.getUniqueName(),
				// d.getCaption());
				// }
