...
public class TotalsListsBuilder implements FormatList {
  ...
  public TotalsListsBuilder(Measure[] selectedMeasures, TotalAggregator[] aggrTempl, CellSet cellSet,
    ...
    if (aggrTempl[0] == null) {
      // use BlankAggregator if one off measure has aggregator but total aggregator
      // not selected
    ...
    TotalNode rootNode = new TotalNode(measuresCaptions, measures, aggrTempl[0], this,
        totalsAxisInfo.fullPositions.size(), dataAxisInfo);
  ...
  private Format getMeasureFormat(Measure m) {
    try {
      ...
      if (StringUtils.isBlank(formatString)) {
        ...
        if (StringUtils.isNotBlank(formatString) && formatString.length() > 1 && formatString.startsWith("\"")
  ...
  private void traverse(List<Integer>[] levels, List<TotalNode>[] totalLists) {
    ...
    nextpos: for (final Position p : dataAxisInfo.axis.getPositions()) {
      int depth = 1;
      int mI = 0;
      ...
      for (int i = changedFrom; i < totalBranch.length; i++) {
        ...
        if (level != null && level.getAggregators() != null && !level.getAggregators().isEmpty()) {
          ...
          for (int j = 0; j < newMeasures.length; j++) {
            if (j < lvlAggr.size()) {
              ThinMeasure mockMeasure = new ThinMeasure(measures[j].getName(), measures[j].getUniqueName(),
                  measures[j].getCaption(), ThinMeasure.Type.EXACT);
  ...
  private Cell getCellAt(int axisCoord, int perpAxisCoord) {
    final Position[] positions = new Position[] { dataAxisInfo.fullPositions.get(axisCoord),
        totalsAxisInfo.fullPositions.get(perpAxisCoord) };
