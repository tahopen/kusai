...
public class ClassPathRepositoryManager implements IRepositoryManager {
    ...
    public static synchronized ClassPathRepositoryManager getClassPathRepositoryManager(String data, String defaultRole,
            ScopedRepo sessionRegistry, boolean workspaces) {
        if (ref == null)
        ...
        if (session == null) {
            ...
            if (new File(append, "etc/license.lic").exists()) {
                try {
                    ...
                    try {
                        FileUtils.copyFile(new File(append, "unknown/etc/license.lic"),
                                this.createNode("/etc/license.lic"));
            ...
            try {
                if (!file.isHidden()) {
                    ...
                    if (relativePath.startsWith(datadir) && datadir.length() >= 0) { // If we have an absolute path
                        relativePath = relativePath.substring(datadir.length(), relativePath.length()); // Make it
                                                                                                        // relative to
                                                                                                        // the datadir
                    }
                ...
                // If a problem happens when handling one file, it will still return the
                // repoObjects list
                ex.printStackTrace();
            }
        }

        // Just after it has filled the repoObjects, sort it alphabetically, putting the
        // directories first
