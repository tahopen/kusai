 ...
 */
package org.kusai.olap.discover;

import org.kusai.datasources.connection.IConnectionManager;
import org.kusai.olap.dto.*;
import org.kusai.olap.util.ObjectUtil;
import org.kusai.olap.util.SaikuCubeCaptionComparator;
import org.kusai.olap.util.SaikuDimensionCaptionComparator;
import org.kusai.olap.util.exception.SaikuOlapException;
import org.kusai.service.util.MondrianDictionary;
...
public class OlapMetaExplorer {
	...
	public List<SimpleCubeElement> getAllMembers(SaikuCube cube, String hierarchy, String level, String searchString,
			...
			if (h != null) {
				...
				if (isMondrian(nativeCube)) {
					if (SaikuMondrianHelper.hasAnnotation(l, MondrianDictionary.SQLMemberLookup)) {
						if (search) {
							ResultSet rs = SaikuMondrianHelper.getSQLMemberLookup(con,
									MondrianDictionary.SQLMemberLookup, l, searchString);
