...
public class BasicRepositoryResource2 implements ISaikuRepository {
	 ...
	 * 
	 * @see
	 * org.kusai.web.rest.resources.ISaikuRepository#getRepository(java.lang.String,
	 * java.lang.String)
	 */
	@GET
	...
	@Produces({ "application/json" })
	@Path("/resource/acl")
	@ReturnType("org.kusai.repository.AclEntry")
	...
	public Response saveResource(
		...
		if (resp.equals("Save Okay")) {
			...
			return Response.serverError().entity("Cannot save resource to ( file: " + file + ")").type("text/plain")
					.build();
		}
		/*
	...
	public Response moveResource(@FormParam("source") String source, @FormParam("target") String target) {
		 ...
		 * throw new
		 * IllegalArgumentException("Path cannot be null or start with \"/\" or \".\" - Illegal Path: "
		 * + source);
		 * }
		 ...
		 * return Response.serverError().status(Status.FORBIDDEN).
		 * entity("You don't have permissions to read the source file: " +
		 * source).build();
		 * }
		 * 
